...
public class CMSAppBase
{
    ...
    public static void CMSApplicationEnd(object sender, EventArgs e)
    {
        ...
        using (var h = CMSApplicationEvents.End.StartEvent(e))
        {
            if (h.Continue)
            {
                ...
                if (WebSyncHelperClass.DeleteGeneratedWebFarmServers)
                {
        ...
        using (var h = CMSRequestEvents.Begin.StartEvent(e))
        {
            if (h.Continue)
            {
                ...
                if (ConnectionAvailable)
                {
                    ...
                    {
                        CMSConnectionScope.EnsureRequestScope(null, false, ConnectionHelper.KeepContextConnectionOpen);
                    }
    ...
    private static void BeforeBeginRequest(object sender, EventArgs e)
    {
        ...
        if (!String.IsNullOrEmpty(connStringPrefix))
        {
            ...
            SqlHelper.ConnectionStringName = connString;
            SqlHelper.ConnectionStringPrefix = connStringPrefix;
        }
    ...
    {
        IHttpHandler handler = URLRewriter.UseHandler;
        if (handler != null)
        {
        ...
        using (var h = CMSRequestEvents.End.StartEvent(e))
        {
            if (h.Continue)
            {
                ...
                if (ConnectionAvailable)
                {
                    ...
                    // Log page view
                    if (RequestHelper.LogPageHit && (!QueryHelper.GetBoolean(URLHelper.SYSTEM_QUERY_PARAMETER, false)) && (status != URLRewritingResultEnum.RESTService))
                    {
                        string siteName = CMSContext.CurrentSiteName;
                        UserInfo currentUser = CMSContext.CurrentUser;
                        PageInfo currentPage = CMSContext.CurrentPageInfo;
    ...
    public static void CMSPreSendRequestHeaders(Object sender, EventArgs e)
    {
        ...
        if (CMSApplicationModule.IsIntegratedMode && !SecurityHelper.IsXFrameOptionsExcluded(URLHelper.CurrentRelativePath, URLHelper.Url.ToString()))
        {
    ...
    private static void DBSeparationCheck()
    {
        ...
        string sepConnString = DBSeparationHelper.ConnStringSeparate;
        bool isSepConnString = !String.IsNullOrEmpty(sepConnString);

        if (isSepConnString)
        {
            // Check separated database
            if (!DBSeparationHelper.CheckCMDatabase(sepConnString))
            {
        ...
        else if (!DBSeparationHelper.CheckCMDatabase(null))
        {
            ...
            (status == URLRewritingResultEnum.MVCPage))
        {
            ...
            if (HttpContext.Current.Session != null)
            {
                ...
                if (!OutputFilter.OutputFilterEndRequestRequired)
                {
                    ...
                    string relativePath = HttpContext.Current.Request.AppRelativeCurrentExecutionFilePath.ToLowerCSafe();
    ...
    private static void LogLastApplicationError()
    {
        if (ConnectionAvailable)
        {
            if ((HttpContext.Current != null) && RequestHelper.LogCurrentError)
            {
                        ...
                        try
                        {
                            ...
                            if (logException)
                            {
                                ...
                                try
                                {
                                    ...
                                    eProvider.LogEvent(eventType, DateTime.Now, "Application_Error", eventCode, CMSContext.CurrentUser.UserID, CMSContext.CurrentUser.UserName, (CMSContext.CurrentDocument != null) ? CMSContext.CurrentDocument.NodeID : 0, (CMSContext.CurrentDocument != null) ? CMSContext.CurrentDocument.GetDocumentName() : null, HTTPHelper.UserHostAddress, EventLogProvider.GetExceptionLogMessage(ex), CMSContext.CurrentSiteID, HTTPHelper.GetAbsoluteUri());
                                }
                                catch
                                {
                                    // can't write to log, do not process any code
