...
public static class Functions
{
    ...
    public static void RedirectToAccessDenied(string resourceName, string permissionName)
    {
        ...
        {
            URLHelper.Redirect(UIHelper.GetAccessDeniedUrl(resourceName, permissionName, null));
        }
    }
    ...
    #region "UniGrid transformations"

    /// <summary>
    /// Gets formatted user name by user ID
    /// </summary>
    /// <param name="parameter">User ID</param>
    private static object UniGridGetFormattedUserName(object parameter)
    {
        int userId = ValidationHelper.GetInteger(parameter, 0);
        UserInfo userInfo = UserInfoProvider.GetUserInfo(userId);
        if (userInfo != null)
        {
            return HTMLHelper.HTMLEncode(GetFormattedUserName(userInfo.UserName, userInfo.FullName));
        }
    ...
    /// <summary>
    /// Registers the functions within the system
    /// </summary>
    public static void RegisterUniGridTransformations()
    {
        UniGridTransformations.Global.RegisterTransformation("#formattedusername", UniGridGetFormattedUserName);
    }
