function ChatErrorsWP(opt) {
    ...
    var chatErrorTemplateB = "chatErrorTemplateB_" + this.Options.clientID,
        ...
        ID = 0,
        cont = jQuery(that.Options.contentClientID),
    ...
    jQuery.template(chatErrorTemplateB, this.Options.errorTemplate);
    jQuery.template(chatBtnTemplateB, this.Options.showDeleteAll);
    ...
    this.ProcessResponse = function (response) {
        ...
        if (that.Options.showDeleteAll.length > 0) {
            if (cont.html().search("ChatError_" + that.Options.clientID) == -1) {
                jQuery.tmpl(chatBtnTemplateB, { "DeleteAll": getWebpart + ".Clear(); return false;" }).prependTo(that.Options.contentClientID);
            }
        }
        ...
        var errorDiv = jQuery('<div id="' + errorID + '" class="ChatError"></div>');
        jQuery.tmpl(chatErrorTemplateB, error).prependTo(errorDiv);
