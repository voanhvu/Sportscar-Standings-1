...
function ChatOnlineUsersWP(opt) {
    ...
    jQuery.template(templateID, "<div class=\"ChatOnlineUser\">" + that.Options.onlineUserTemplate + "</div>");
    ...
    function RenderUsers(users) { 
        ...
        for (var i = 0; i < users.length; i++) {
            var usr = users[i];
            usr.IsCurrentUser = (usr.ChatUserID == jQuery.ChatManager.Login.UserID) ? true : false;
            if (that.Options.inviteMode == true) {
            ...
            jQuery.tmpl(templateID, usr).appendTo(content);
        }
