...
public partial class CMSWebParts_Membership_Logon_currentuser : CMSAbstractWebPart
{
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            if (ShowUserFullName)
            {
                ...
                // Display user name
                if (ShowUserName && !(CMSContext.CurrentUser.IsExternal && HideUserNameForExternalUsers))
                {
                    userInfo += " (" + HTMLHelper.HTMLEncode(Functions.GetFormattedUserName(CMSContext.CurrentUser.UserName, true)) + ") ";
                }
            }
            // Display user name
            else if (ShowUserName && !(CMSContext.CurrentUser.IsExternal && HideUserNameForExternalUsers))
            {
                userInfo += HTMLHelper.HTMLEncode(Functions.GetFormattedUserName(CMSContext.CurrentUser.UserName, true));
            }
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        Visible = (!ShowOnlyWhenAuthenticated || !CMSContext.CurrentUser.IsPublic());
    }
}