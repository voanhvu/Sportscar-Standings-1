...
public partial class CMSWebParts_MessageBoards_MessageBoard : CMSAbstractWebPart
{
    ...
    public new UserInfo CurrentUser
    {
        get
        {
            ...
            {
                mCurrentUser = SiteContext.CurrentUser;
            }
            ...
            if (mBoardObj == null)
            {
                switch (BoardOwner.ToLowerCSafe())
                {
                    case "user":
                        ...
                        if (CurrentUser != null)
                        {
                            mBoardObj = BoardInfoProvider.GetBoardInfoForUser(CurrentUser.UserID, GetBoardName(WebPartName, "user"));
                        }
                        break;

                    default:
                        // If the 'BoardType' is of 'document' type or isn't specified use the default way to get the board info
                        mBoardObj = BoardInfoProvider.GetBoardInfo(GetBoardName(WebPartName, ""), CMSContext.CurrentPageInfo.DocumentID);
                        if (mBoardObj == null)
                        {
                            // Backward compatibility
                            mBoardObj = BoardInfoProvider.GetBoardInfo(WebPartName + "_doc_" + CMSContext.CurrentPageInfo.NodeGUID, CMSContext.CurrentPageInfo.DocumentID);
                        }
                        break;
                }
            }
    ...
    private void SetupControls()
    {
        ...
        else
        {
            ...
            if (BoardObj != null)
            {
                ...
                msgBoard.BoardProperties.BoardUnsubscriptionUrl = BoardInfoProvider.GetUnsubscriptionUrl(BoardObj.BoardUnsubscriptionURL, CMSContext.CurrentSiteName);
                msgBoard.BoardProperties.BoardBaseUrl = (string.IsNullOrEmpty(BoardObj.BoardBaseURL)) ? ValidationHelper.GetString(SettingsKeyProvider.GetStringValue(CMSContext.CurrentSiteName + ".CMSBoardBaseURL"), "") : BoardObj.BoardBaseURL;
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        boardDisplayName = CMSContext.CurrentPageInfo.GetDocumentName() + " (" + CMSContext.CurrentPageInfo.DocumentNamePath + ")";
                    }
                    // Limit display name length
                    msgBoard.BoardProperties.BoardDisplayName = TextHelper.LimitLength(boardDisplayName, 250, "");

                    msgBoard.BoardProperties.BoardUnsubscriptionUrl = BoardInfoProvider.GetUnsubscriptionUrl(BoardUnsubscriptionUrl, CMSContext.CurrentSiteName);
                    msgBoard.BoardProperties.BoardBaseUrl = (string.IsNullOrEmpty(BoardBaseUrl)) ? ValidationHelper.GetString(SettingsKeyProvider.GetStringValue(CMSContext.CurrentSiteName + ".CMSBoardBaseURL"), "") : BoardBaseUrl;
