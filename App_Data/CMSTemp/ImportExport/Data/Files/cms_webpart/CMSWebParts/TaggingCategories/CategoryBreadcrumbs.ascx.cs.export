...
public partial class CMSWebParts_TaggingCategories_CategoryBreadcrumbs : CMSAbstractWebPart
{
    ...
    protected void SetupControl()
    {
        if (!StopProcessing)
        {
            ...
            // Hide control when category check failed and hidding is enabled
    ...
    protected string CreateCategoryPartLink(int categoryId)
    {
        ...
        if (category != null)
        {
            ...
            return string.Format("<a href=\"{0}\"{1}>{2}</a>", URLHelper.EncodeQueryString(url), attrs, FormatCategoryDisplayName(categoryDisplayName, true));
        }
