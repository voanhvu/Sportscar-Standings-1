...
BEGIN
	...
	BEGIN		
		EXEC Proc_Analytics_GenerateSampleData @From,@To,@SiteID,'visitfirst'
	END
	...
	INSERT INTO @CodeNames VALUES ('crawler',2) -- 26
	INSERT INTO @CodeNames VALUES ('mobiledevice',2) -- 27
	INSERT INTO @CodeNames VALUES ('bannerclick',0) -- 28
	INSERT INTO @CodeNames VALUES ('bannerhit',0) -- 29
	-- Cultures 
	...
	-- Mobile device
	INSERT INTO @AnalyticsDetail VALUES (27,'Asus Eee Pad Transformer',null)
	INSERT INTO @AnalyticsDetail VALUES (27,'Apple iPad',null)
	INSERT INTO @AnalyticsDetail VALUES (27,'Apple iPhone',null)
	INSERT INTO @AnalyticsDetail VALUES (27,'Unknown',null)
	
	DECLARE @enginecrawler nvarchar(200)
	-- Crawler
	IF ((SELECT COUNT(*) FROM CMS_SearchEngine WHERE ISNULL(SearchEngineCrawler,'') != '') > 0) BEGIN
	
		SET @enginecrawler = (SELECT TOP 1 SearchEngineName FROM CMS_SearchEngine WHERE ISNULL(SearchEngineCrawler,'') != '');
		INSERT INTO @AnalyticsDetail VALUES(26, @enginecrawler, null) 
	END
	
	IF ((SELECT COUNT(*) FROM CMS_SearchEngine WHERE ISNULL(SearchEngineCrawler,'') != '') > 1) BEGIN
		SET @enginecrawler = (SELECT TOP 1 SearchEngineName FROM CMS_SearchEngine WHERE ISNULL(SearchEngineCrawler,'') != '' AND SearchEngineName NOT IN (SELECT ObjectName FROM @AnalyticsDetail));
		INSERT INTO @AnalyticsDetail VALUES(26, @enginecrawler, null) 
	END
	
	IF ((SELECT COUNT(*) FROM CMS_SearchEngine WHERE ISNULL(SearchEngineCrawler,'') != '') > 2) BEGIN
		SET @enginecrawler = (SELECT TOP 1 SearchEngineName FROM CMS_SearchEngine WHERE ISNULL(SearchEngineCrawler,'') != '' AND SearchEngineName NOT IN (SELECT ObjectName FROM @AnalyticsDetail));
		INSERT INTO @AnalyticsDetail VALUES(26, @enginecrawler, null)
	END
	...
	-- BANNAR MANAGEMENT
	DECLARE @banner int
	SET @banner = (SELECT TOP 1 BannerID FROM CMS_Banner WHERE BannerSiteID = @SiteID) 
	IF (@banner IS NOT NULL)
	BEGIN
		INSERT INTO @AnalyticsDetail VALUES(28, null, @banner) 
		INSERT INTO @AnalyticsDetail VALUES(29, null, @banner) 
	END
