CREATE PROCEDURE [Proc_CMS_PageTemplateCategory_OrderAlphaAsc] 
...
BEGIN
	...
	/* Get the nodes list */
	INSERT INTO @categoriesTable SELECT CategoryID, CategoryOrder, CategoryDisplayName FROM CMS_PageTemplateCategory WHERE CategoryParentID = @CategoryParentID;
	...
	BEGIN
		...
		IF @currentCategoryOrder IS NULL OR @currentCategoryOrder <> @currentIndex
			UPDATE CMS_PageTemplateCategory SET CategoryOrder = @currentIndex WHERE CategoryID = @currentCategoryId;
		/* Get next record */
