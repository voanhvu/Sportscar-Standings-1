...
-- SET NOCOUNT ON added to prevent extra result sets from
	...
	-- delete report's subscription 
	DELETE FROM [Reporting_ReportSubscription] WHERE ReportSubscriptionSiteID=@ID
    ...
    -- deletes site's ObjectWorkflowTriggers
    DELETE FROM [CMS_ObjectWorkflowTrigger] WHERE TriggerSiteID=@ID;
    ...
    -- NEWSLETTER
    -- deletes [Newsletter_Emails]
    DELETE FROM [Newsletter_Emails] WHERE EmailSiteID=@ID;
    
    -- deletes [Newsletter_ABTest]
    ...
    DELETE FROM [Newsletter_NewsletterIssue] WHERE IssueSiteID=@ID;
    ...
    -- deletes [Newsletter_EmailTemplateNewsletter] - newsletters' additional templates
    DELETE FROM [Newsletter_EmailTemplateNewsletter] WHERE NewsletterID IN (
		SELECT NewsletterID FROM [Newsletter_Newsletter] WHERE NewsletterSiteID=@ID
    );
    ...
    DELETE FROM Media_File WHERE FileSiteID = @ID;
    ...
    -- Class mappings
    DELETE FROM [CMS_ClassMapping] WHERE MappingSiteID = @ID;
    
    -- Banner management
    DELETE FROM [CMS_Banner] WHERE BannerCategoryID IN (SELECT BannerCategoryID FROM [CMS_BannerCategory] WHERE [BannerCategorySiteID] = @ID)
    DELETE FROM [CMS_BannerCategory] WHERE [BannerCategorySiteID] = @ID;
    
    -- Translation submissions
    DELETE FROM [CMS_TranslationSubmissionItem] WHERE SubmissionItemSubmissionID IN (SELECT SubmissionID FROM [CMS_TranslationSubmission] WHERE SubmissionSiteID = @ID);
    DELETE FROM [CMS_TranslationSubmission] WHERE SubmissionSiteID = @ID;
   
    -- Chat
    DELETE FROM [Chat_SupportTakenRoom] WHERE ChatSupportTakenRoomRoomID IN (SELECT [ChatRoomID] FROM [Chat_Room] WHERE ChatRoomSiteID = @ID);
    DELETE FROM [Chat_Notification] WHERE ChatNotificationRoomID IN (SELECT [ChatRoomID] FROM [Chat_Room] WHERE ChatRoomSiteID = @ID);
    DELETE FROM [Chat_RoomUser] WHERE ChatRoomUserRoomID IN (SELECT [ChatRoomID] FROM [Chat_Room] WHERE ChatRoomSiteID = @ID);
    DELETE FROM [Chat_Message] WHERE ChatMessageRoomID IN (SELECT [ChatRoomID] FROM [Chat_Room] WHERE ChatRoomSiteID = @ID);
    DELETE FROM [Chat_InitiatedChatRequest] WHERE InitiatedChatRequestRoomID IN (SELECT [ChatRoomID] FROM [Chat_Room] WHERE ChatRoomSiteID = @ID);
    DELETE FROM [Chat_Room] WHERE ChatRoomSiteID = @ID;
    DELETE FROM [Chat_OnlineUser] WHERE ChatOnlineUserSiteID = @ID;
	
	DELETE FROM [Chat_Notification] WHERE ChatNotificationSiteID = @ID;
	DELETE FROM [Chat_SupportCannedResponse] WHERE ChatSupportCannedResponseSiteID = @ID;
	DELETE FROM [Chat_OnlineSupport] WHERE ChatOnlineSupportSiteID = @ID;
	-- Automation state
	DELETE FROM [CMS_AutomationHistory] WHERE HistoryStateID IN (SELECT StateID FROM [CMS_AutomationState] WHERE StateSiteID = @ID);
	DELETE FROM [CMS_AutomationState] WHERE StateSiteID = @ID;
END
