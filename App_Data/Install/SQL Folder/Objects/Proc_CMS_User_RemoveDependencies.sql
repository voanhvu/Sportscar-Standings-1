...
BEGIN
	...
	-- deletes dependencies in CMS_UserCulture
    DELETE FROM [CMS_UserCulture] WHERE UserID=@ID;
	-- deletes dependencies in Forums_ForumModerators
    DELETE FROM [Forums_ForumModerators] WHERE UserID=@ID;
	-- deletes dependencies in Forums_ForumSubscription
    ...
    DELETE CMS_WorkflowUser WHERE UserID=@ID;
    -- remove dependencies for workflow step
    DELETE FROM CMS_WorkflowStepUser WHERE UserID=@ID; 
    ...
    -- update dependencies in COM_Customer, COM_OrderStatusUser, COM_ShoppingCart, COM_Order
    UPDATE COM_Customer SET CustomerUserID=NULL WHERE CustomerUserID=@ID;
    UPDATE COM_Order SET OrderCreatedByUserID=NULL WHERE OrderCreatedByUserID=@ID;
    ...
    DELETE FROM [Reporting_ReportSubscription] WHERE ReportSubscriptionUserID = @ID    
    ...
    -- Ad-hoc tasks
    UPDATE [PM_ProjectTask] SET ProjectTaskOwnerID = COALESCE(ProjectTaskCreatedByID, ProjectTaskAssignedToUserID, ProjectTaskOwnerID) WHERE (ProjectTaskOwnerID = @ID) AND (ProjectTaskProjectID IS NULL);
    UPDATE [PM_ProjectTask] SET ProjectTaskAssignedToUserID = COALESCE(ProjectTaskOwnerID, ProjectTaskCreatedByID, ProjectTaskAssignedToUserID) WHERE (ProjectTaskAssignedToUserID = @ID) AND (ProjectTaskProjectID IS NULL);
	...
	-- Object settings
	UPDATE [CMS_ObjectSettings] SET [ObjectCheckedOutByUserID] = NULL WHERE [ObjectCheckedOutByUserID] = @ID; 
    -- Scheduled tasks
    UPDATE [CMS_ScheduledTask] SET [TaskUserID] = NULL WHERE [TaskUserID] = @ID
    
    -- Translation submissions
    DELETE FROM [CMS_TranslationSubmissionItem] WHERE SubmissionItemSubmissionID IN (SELECT SubmissionID FROM [CMS_TranslationSubmission] WHERE SubmissionSubmittedByUserID = @ID);
    DELETE FROM [CMS_TranslationSubmission] WHERE SubmissionSubmittedByUserID = @ID;
    
    -- Chat - convert user to anonymous
    UPDATE [Chat_User] SET [ChatUserUserID] = NULL WHERE [ChatUserUserID] = @ID;
    DELETE FROM [Chat_InitiatedChatRequest] WHERE [InitiatedChatRequestUserID] = @ID;
