...
/* Declare the selection table */
	...
	INSERT INTO @stepsTable SELECT StepID, StepOrder, StepName FROM CMS_WorkflowStep WHERE StepWorkflowID = @StepWorkflowID AND (StepType IS NULL OR StepType = 3);
	...
	UPDATE CMS_WorkflowStep SET StepOrder = 1 WHERE StepWorkflowID = @StepWorkflowID AND StepType = 2;
	...
	UPDATE CMS_WorkflowStep SET StepOrder = @currentIndex WHERE StepWorkflowID = @StepWorkflowID AND StepType = 100;
	SET @currentIndex = @currentIndex + 1;
	/* Set the archived step */
	UPDATE CMS_WorkflowStep SET StepOrder = @currentIndex WHERE StepWorkflowID = @StepWorkflowID AND StepType = 101;
END
