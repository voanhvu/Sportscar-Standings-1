...
function AC_ReceiveAsyncStatus_", ClientID, @"(rvalue, context) {
    ...
    if ( code == 'running') { 
        ...
        ", Page.ClientScript.GetPostBackEventReference(this, "finished"), @";
    }
        ...
        asyncProcessFinished_", ClientID, @" = true;
        ", (PostbackOnError ? (Page.ClientScript.GetPostBackEventReference(this, "error") + ";") : "AC_SetClose_" + ClientID + "();"), @"
    }
}");
...
timeout_", ClientID, " = setTimeout(function() {AC_GetAsyncStatus_", ClientID, "();}, ", TIMEOUT, ");");
    ...
    public void RaiseCallbackEvent(string eventArgument)
    {
        ...
        if (SqlHelperClass.MachineName.ToLowerCSafe() == args[1])
        {
            if (WorkerExists())
            {
