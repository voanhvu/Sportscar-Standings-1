...
function search(editor, text, matchCase) {
    ...
    for (var cursor = editor.getSearchCursor(text, findPos, !matchCase); cursor.findNext(); ) {
        ...
        marked.push(editor.markText(from, to, "CodeMirror-searched"));
        return true;
    }
...
function doResize() {
    ...
    txtSource.style.height = txtSource.style.maxHeight = (container.clientHeight - 6) + 'px';
    txtSource.style.width = txtSource.style.maxWidth = (container.clientWidth - 6) + 'px';
