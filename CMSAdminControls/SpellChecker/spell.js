    ...
    try {
        if (typeof (CKEDITOR) !== 'undefined') {
            for (var name in CKEDITOR.instances) {
                if ((checkEditors.length > 0) && (checkEditors[0] == CKEDITOR.instances[name])) {
                    break;
                }
...
function openSpellChecker() {
    ...
    if (window.modalDialog)
        var result = modalDialog(spellURL, "spellcheck", 400, 460);
    else
        var newWindow = window.open(spellURL, "newWindow", "height=" + 460 + ",width=400,scrollbars=no,resizable=no,toolbars=no,status=no,menubar=no,location=no");
}
...
function initialize(wordIndexID, currentTextID, elementIndexID, spellModeID, statusTextID, replacementWordID) {
...
function loadText() {
    ...
    for (++iElementIndex; iElementIndex < parentWindow.checkElements.length + parentWindow.checkEditors.length; iElementIndex++) {
        ...
        if ((newText != null) && (newText.length > 0)) {
            ...
            statusTextElem.innerText = "Spell Checking Text ...";
            return true;
        }
    }
...
//Global settings for function CMSHandleLabel used in function endCheck
var posOffsetX = 20;
var posOffsetY = 0;
var lblsOpacity = 100;
var wrpCtrlid = '';
...
function endCheck() {
    if (showCompleteAlert) {
        CMSHandleLabel('msgSpellComplete', '', 2000, 0);
    }
}
