        ...
        if (rule.IsLeaf)
        {
            ...
            if (rule.Parameters != null)
            {
                ...
                foreach (string key in rule.Parameters.Keys)
                {
                    ...
                    paramText = MacroRuleTree.GetParameterText(paramText, true);

                    text = Regex.Replace(text, "\\{" + key + "\\}", "</span><span class=\"MacroRuleParameter\" onclick=\"ChangeParamValue('" + rule.IDPath + "', " + ScriptHelper.GetString(key) + ");\">" + paramText + "</span>" + handleParams, RegexOptions.IgnoreCase);
                    i++;
                }
            }
