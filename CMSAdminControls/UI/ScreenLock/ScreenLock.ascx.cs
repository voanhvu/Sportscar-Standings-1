...
public partial class CMSAdminControls_UI_ScreenLock_ScreenLock : CMSUserControl, ICallbackEventHandler
{
    ...
    int minutesToLock = SettingsKeyProvider.GetIntValue(CMSContext.CurrentSiteName + ".CMSScreenLockInterval");
    int secondsToWarning = SettingsKeyProvider.GetIntValue(CMSContext.CurrentSiteName + ".CMSScreenLockWarningInterval");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!RequestHelper.IsCallback())
        {
            ...
            string clientScript = "function serverRequest(args, context){{ " +
                ...
                "; }}" +
                "var screenLockEnabled = " + SecurityHelper.IsScreenLockEnabled(CMSContext.CurrentSiteName).ToString().ToLowerCSafe() + ";";
