...
public partial class CMSAdminControls_UI_Selectors_SelectValidity : FormEngineUserControl
{
    ...
    private void InitByForm()
    {
        ...
        if (Form.Data.ContainsColumn(ValidForFieldName))
        {
            ValidFor = Form.Data.GetValue(ValidForFieldName) as int?;
        }

        if (Form.Data.ContainsColumn(ValidUntilFieldName))
        {
            ValidUntil = Form.Data.GetValue(ValidUntilFieldName) as DateTime?;
        }
    }
