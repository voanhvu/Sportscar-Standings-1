...
public partial class CMSAdminControls_UI_WebServiceChecker : CMSUserControl, IPostBackEventHandler
{
    ...
    public string NotFoundText
    {
        get
        {
            string link = String.Format("<a target=\"_blank\" href=\"{0}\">{1}</a>", ResolveUrl(UIHelper.GetContextHelpUrl("configuring_wcf")), GetString("general.ourdocumentation"));
    ...
    public string ServerErrorText
    {
        get
        {
            string link = String.Format("<a target=\"_blank\" href=\"{0}\">{1}</a>", ResolveUrl(UIHelper.GetContextHelpUrl("configuring_wcf_https")), GetString("general.ourdocumentation"));
