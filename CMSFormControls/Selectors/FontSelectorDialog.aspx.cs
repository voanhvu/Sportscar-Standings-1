...
public partial class CMSFormControls_Selectors_FontSelectorDialog : CMSModalPage
{
    ...
    private void RegisterScriptCode()
    {
        ...
        function btOk_clicked() {
            ...
            if (size == '' || type  == '' || style == '' ) {
                alert('" + GetString("fontselector.notallvaluesfilled") + @"');   
                return false;                
            }
            return true;                
        }
        ...
        function sizeManualUpdate(tbInput) {
            ...
            if (val != parseInt(val) || val == 0)   {
                alert ('" + GetString("fontselector.wrongsize") + @"');                
    ...
    protected void btnOk_Click(object sender, EventArgs e)
    {
        if (txtFontSize.Text != String.Empty && txtFontStyle.Text != String.Empty && txtFontType.Text != String.Empty)
        {
            ...
            try
            {
                Font ft = new Font(txtFontType.Text, ValidationHelper.GetInteger(txtFontSize.Text, 10), fs);
            }
            catch
            {
            ...
            string submitScript = ScriptHelper.GetScript(String.Format("wopener.getParameters('{0}','{1}','{2}'); CloseDialog();", ret, mHiddenFieldID, mFontTypeID));
