...
public partial class CMSFormControls_System_OptionsSelector : FormEngineUserControl
{
    ...
    {
        base.OnPreRender(e);
        if (Form.FormType == FormTypeEnum.BizForm)
        {
    ...
    public override bool IsValid()
    {
                ...
                for (lineIndex = 0; lineIndex <= lines.GetUpperBound(0); lineIndex++)
                {
                    ...
                    if ((lines[lineIndex] != null) && (lines[lineIndex].Trim() != ""))
                    {
                        ...
                        else
                        {
                            ...
                            if (FieldInfo.DataType == FormFieldDataTypeEnum.Decimal)
                            {
                            ...
                            else if (FieldInfo.DataType == FormFieldDataTypeEnum.Integer)
                            {
                            ...
                            else if (FieldInfo.DataType == FormFieldDataTypeEnum.LongInteger)
                            {
        ...
        {
            ValidationError += String.Format(GetString("formcontrol.missingcolumn"), "options", GetString("templatedesigner.fieldtypes.longtext"));
            valid = false;
        }

        if (!ContainsColumn("query"))
        {
            ValidationError += String.Format(GetString("formcontrol.missingcolumn"), "query", GetString("templatedesigner.fieldtypes.longtext"));
            valid = false;
        }
    ...
    {
        string query = ValidationHelper.GetString(Form.Data.GetValue("query"), "").Trim();
