    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            if (WhereCondition == String.Empty)
            {
                ...
                if (si != null)
                {
                    ucAbuseReportGrid.WhereCondition = SqlHelperClass.AddWhereCondition(ucAbuseReportGrid.WhereCondition, "(ReportSiteID = " + si.SiteID + ")");
                }

                // Status
                if (Status != -1)
                {
                    ucAbuseReportGrid.WhereCondition = SqlHelperClass.AddWhereCondition(ucAbuseReportGrid.WhereCondition, "(ReportStatus = " + Status + ")");
                }
            }
            else
            {
