...
public partial class CMSModules_AdminControls_Controls_Class_ClassSites : CMSUserControl
{
    ...
    {
        DataSet ds = ClassSiteInfoProvider.GetClassSites("SiteID", "ClassID = " + ClassId, null, 0);
        ...
        if (!String.IsNullOrEmpty(items))
        {
            ...
            if (newItems != null)
            {
                ...
                foreach (string item in newItems)
                {
                    ...
                    {
                        // Check license
                        if (CheckLicense && !CustomTableItemProvider.LicenseVersionCheck(si.DomainName, FeatureEnum.CustomTables, VersionActionEnum.Insert))
                        {
