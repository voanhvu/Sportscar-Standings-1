...
public partial class CMSModules_AdminControls_Controls_Class_QueryEdit : CMSUserControl
{
    ...
    protected void btnGenerate_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            txtQueryText.Text = SqlGenerator.GetSqlQuery(ClassName, QueryProvider.GetQueryType(QueryName), null);
        }
        catch (Exception ex)
        {
    ...
    private bool SaveQuery()
    {
        ...
        if (!QueryProvider.IsSystemQuery(QueryName) && !dci.ClassIsDocumentType && !dci.ClassIsCustomTable && !dci.ClassShowAsSystemTable)
        {
