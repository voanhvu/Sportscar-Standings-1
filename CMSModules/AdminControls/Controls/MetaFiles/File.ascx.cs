...
public partial class CMSModules_AdminControls_Controls_MetaFiles_File : ReadOnlyFormEngineUserControl
{
    ...
    protected bool AllowModify
    {
        get
        {
            return Enabled && UserInfoProvider.IsAuthorizedPerObject(ObjectType, ObjectID, PermissionsEnum.Modify, CMSContext.CurrentSiteName, CMSContext.CurrentUser);
        }
    }
            ...
            case "update":
                {
                        ...
                        && WebDAVSettings.IsExtensionAllowedForEditMode(fileExtension, siteName))
                    {
                        ...
                        WebDAVEditControl webDAVElem = Page.LoadUserControl("~/CMSModules/WebDAV/Controls/MetaFileWebDAVEditControl.ascx") as WebDAVEditControl;
            ...
            using (DataSet ds = MetaFileInfoProvider.GetMetaFiles(ObjectID, ObjectType, Category, null, null))
            {
                if (!DataHelper.DataSourceIsEmpty(ds))
                {
                    foreach (DataRow dr in ds.Tables[0].Rows)
                    {
                        ...
                        if ((mfi != null) && (mfi.MetaFileName.ToLowerCSafe() == uploader.CurrentFileName.ToLowerCSafe()))
                        {
...
function OpenEditor(queryString) {{ 
    ...
    return false; 
        ...
        if (ObjectID > 0)
        {
            ...
            plcUploader.Visible = (AllowModify && !gridHasData);
            plcUploaderDisabled.Visible = (!AllowModify && !gridHasData);
        }
    }
    ...
    public override bool IsValid()
    {
        ...
        {
            string extensions = string.IsNullOrEmpty(AllowedExtensions) ? SettingsKeyProvider.GetStringValue(CMSContext.CurrentSiteName + ".CMSUploadExtensions") : AllowedExtensions;
