...
<asp:Panel runat="server" ID="pnlContent" CssClass="PageContent">
    ...
    <asp:Panel ID="pnlEditList" runat="server">
        <cms:UniGrid ID="gridItems" runat="server" OrderBy="RelationshipNameID" ObjectType="cms.objectrelationship" DelayedReload="true"
            ...
            <GridColumns>
                <ug:Column Source="##ALL##" ExternalSourceName="#transform: {% RelationshipLeftObjectType %} => {% RelationshipLeftObjectID %} : {% TypeInfo.GetNiceObjectTypeName() %} '{% DisplayName %}'" Caption="$ObjRelationship.LeftSide$"
                    Wrap="false" />
                ...
                <ug:Column Source="##ALL##" ExternalSourceName="#transform: {% RelationshipRightObjectType %} => {% RelationshipRightObjectID %} : {% TypeInfo.GetNiceObjectTypeName() %} '{% DisplayName %}'" Caption="$ObjRelationship.RightSide$"
                    Wrap="false" />
                <ug:Column Width="100%" />
            </GridColumns>
