...
public partial class CMSModules_Badges_Badges_List : SiteManagerPage
{
    ...
    {
        // Set the page title
        CurrentMaster.Title.TitleText = GetString("badge.title");
        CurrentMaster.Title.TitleImage = GetImageUrl("Objects/CMS_Badge/object.png");
        CurrentMaster.Title.HelpTopicName = "badge";
    ...
    private object UniGrid_OnExternalDataBound(object sender, string sourceName, object parameter)
    {
        switch (sourceName.ToLowerCSafe())
        {
            case "imageurl":
                ...
                {
                    url = "<img alt=\"Badge image\" src=\"" + GetImageUrl(url) + "\" style=\"max-width:50px; max-height: 50px;\"  />";
                    return url;
                }
                return "";
            case "isautomatic":
