...
public partial class CMSModules_BizForms_Tools_BizForm_Edit_Header : CMSBizFormPage
{
    ...
    protected void InitalizeMenu(int formId)
    {
        ...
        if (user.IsAuthorizedPerResource("cms.form", "ReadForm"))
        {
            ...
            //SetTab(i++, "FormBuilder", "BizForm_Edit_FormBuilder.aspx?formid=" + formId, "SetHelpTopic('helpTopic', 'formbuilder');");
            ...
            if (ModuleEntry.IsModuleLoaded(ModuleEntry.ONLINEMARKETING) && (user.IsAuthorizedPerResource(ModuleEntry.CONTACTMANAGEMENT, "ModifyContacts")) && LicenseHelper.CheckFeature(URLHelper.GetCurrentDomain(), FeatureEnum.ContactManagement))
            {
