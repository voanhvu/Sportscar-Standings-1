...
<script type="text/javascript" language="javascript">
    ...
    function NodeSelected(elementId, parentId, isNew) {
        // Set menu actions value
        var menuElem = $j('#' + menuHiddenId);
        if (menuElem.length = 1) {
            menuElem[0].value = elementId + '|' + parentId + '|' + isNew;
        }
                                    ...
                                    <tbody>
                                        <tr>
                                            <td style="width: 50%; height: 24px;">
                                                <div>
                                                    <asp:Image ID="imgNewCategory" runat="server" CssClass="NewItemImage" /><asp:LinkButton
                                                        ID="btnNew" runat="server" OnClick="btnNewElem_Click" EnableViewState="false" />
                                                </div>
                                            </td>
                                            <td style="width: 50%">
                                                <div>
                                                    <asp:Image ID="imgMoveUp" runat="server" CssClass="NewItemImage" /><asp:LinkButton
                                                        ID="btnUp" runat="server" OnClick="btnUpElem_Click" EnableViewState="false" />
                                                </div>
                                            </td>
                                        </tr>
                                        <tr>
                                            <td style="width: 50%; height: 24px;">
                                                <div>
                                                    <asp:Image ID="imgDeleteCategory" runat="server" CssClass="NewItemImage" /><asp:LinkButton
                                                        ID="btnDelete" runat="server" OnClick="btnDeleteElem_Click" EnableViewState="false"
                                                        OnClientClick="return deleteConfirm();" />
                                                </div>
                                            </td>
                                            <td style="width: 50%">
                                                <div>
                                                    <asp:Image ID="imgMoveDown" runat="server" CssClass="NewItemImage" /><asp:LinkButton
                                                        ID="btnDown" runat="server" OnClick="btnDownElem_Click" EnableViewState="false" />
                                                </div>
                                            </td>
                                        </tr>
                                    </tbody>
        ...
        <cms:CMSUpdatePanel ID="pnlUpdateContent" runat="server" UpdateMode="Conditional">
            <ContentTemplate>
                <asp:Panel ID="pnlHeader" runat="server" CssClass="PageHeader">
                    <cms:PageTitle ID="titleElem" ShortID="pt" runat="server" EnableViewState="false" />
                </asp:Panel>
                <div id="divDialogView" runat="server">
                    ...
                    <asp:PlaceHolder ID="plcNew" runat="server" Visible="false">
                        <cms:CategoryEdit ID="catNew" runat="server" Visible="true" AllowDisabledParents="true"
                            PanelCssClass="PageContent" ComponentName="CategoryNew" />
                    </asp:PlaceHolder>
