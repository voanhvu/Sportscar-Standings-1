...
<cms:UniGrid runat="server" ID="gridElem" ObjectType="chat.message" OrderBy="ChatMessageCreatedWhen DESC"
    ...
    <GridActions Parameters="ChatMessageID;ChatMessageRoomID">
        <ug:Action Name="edit" Caption="$general.edit$" ExternalSourceName="edit" />
        <ug:Action Name="delete" Caption="$general.delete$" ExternalSourceName="delete" Confirmation="$General.ConfirmDelete$" />
        <ug:Action Name="reject" Caption="$general.reject$" ExternalSourceName="reject" />   
    </GridActions>
