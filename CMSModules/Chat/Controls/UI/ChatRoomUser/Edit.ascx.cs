...
public partial class CMSModules_Chat_Controls_UI_ChatRoomUser_Edit : CMSAdminEditControl
{
    ...
    public ChatRoomUserInfo TypedEditedObject
    {
        get
        {
            return (ChatRoomUserInfo)CMSContext.EditedObject;
        }
    }
    ...
    {
        UIFormControl.OnCheckPermissions += new EventHandler(EditForm_OnCheckPermissions);
        UIFormControl.OnBeforeValidate += new EventHandler(EditForm_OnBeforeValidate);
        UIFormControl.OnBeforeSave += new EventHandler(EditForm_OnBeforeSave);
        ...
        if (!RequestHelper.IsPostBack())
        {
            ...
            {
                fdrpAdminLevel.Items.Add(new ListItem(enumValue.ToStringValue(), ((int)enumValue).ToString()));
            }
