    ...
    protected void btnOk_Click(object sender, EventArgs e)
    {
        if (modifyAccountContact)
        {
            ...
            switch (what)
            {
                ...
                // Selected items
                case What.Selected:
                    where = SqlHelperClass.GetWhereCondition<int>("AccountContactID", gridElem.SelectedItems, false);
                    break;
                default:
                    return;
            }
...
{
    modalDialog('", ResolveUrl(CONTACT_DETAIL_DIALOG), @"?contactid=' + contactID + '&isSiteManager=", ContactHelper.IsSiteManager, @"', 'ContactDetail', '1024', '700');
}
function Refresh()
{
