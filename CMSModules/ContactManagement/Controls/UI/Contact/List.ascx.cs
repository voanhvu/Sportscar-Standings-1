...
public partial class CMSModules_ContactManagement_Controls_UI_Contact_List : CMSAdminListControl, ICallbackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        gridElem.WhereCondition = SqlHelperClass.AddWhereCondition(gridElem.WhereCondition, WhereCondition);
        gridElem.EditActionUrl = "Frameset.aspx?contactId={0}&siteId=" + SiteID;
    ...
    protected void btnOk_Click(object sender, EventArgs e)
    {
        ...
        switch (what)
        {
            ...
            // Selected items
            case What.Selected:
                where = SqlHelperClass.GetWhereCondition<int>("ContactID", gridElem.SelectedItems, false);
                break;
        }
            ...
            case Action.AddToGroup:
                ...
                if (groupId > 0)
                {
                    ...
                    if (contactIds != null)
                    {
                        ...
                        if (group != null)
                        {
                            ShowConfirmation(String.Format(GetString("om.contact.massaction.addedtogroup"), ResHelper.LocalizeString(group.ContactGroupDisplayName)));
                        }
                    }
                }
                break;
