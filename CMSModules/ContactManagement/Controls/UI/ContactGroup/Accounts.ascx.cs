...
public partial class CMSModules_ContactManagement_Controls_UI_ContactGroup_Accounts : CMSAdminListControl
{
    ...
    private string GetWhereCondition()
    {
        ...
        if (siteID > 0)
        {
            ...
            {
                CMSPage.RedirectToCMSDeskAccessDenied(ModuleEntry.CONTACTMANAGEMENT, "ReadAccounts");
            }
        }
...
{
    modalDialog('", ResolveUrl(ACCOUNT_DETAIL_DIALOG), @"?accountid=' + accountID ", sitemanagerAppend, @", 'AccountDetail', '1024', '700');
}
function Refresh()
{
