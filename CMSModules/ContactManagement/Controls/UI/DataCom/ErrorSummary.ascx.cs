...
public partial class CMSModules_ContactManagement_Controls_UI_DataCom_ErrorSummary : CMSAdminControl
{
    ...
    private void DisplayError()
    {
        ...
        string tooltip = GetErrorTooltip();
        if (MessagesEnabled)
        {
    ...
    /// </summary>
    /// <returns>An error tooltip.</returns>
    private string GetErrorTooltip()
    {
        if (SettingsKeyProvider.DevelopmentMode && mException != null)
        {
            return HTMLHelper.HTMLEncode(mException.Message ?? mException.ToString());
        }
    ...
    private void LocalizeErrors()
    {
        if (mErrors != null)
        {
            foreach (Error error in mErrors.Where(x => !String.IsNullOrEmpty(x.ErrorCode)))
            {
                ...
                string message = ResHelper.GetString(name, Thread.CurrentThread.CurrentUICulture.Name, String.Empty);
