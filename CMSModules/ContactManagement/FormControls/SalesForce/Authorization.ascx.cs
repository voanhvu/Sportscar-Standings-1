...
public partial class CMSModules_ContactManagement_FormControls_SalesForce_Authorization : FormEngineUserControl, ICallbackEventHandler
{
    ...
    {
        base.OnLoad(e);
        string baseUrl = URLHelper.ResolveUrl("~/CMSModules/ContactManagement/Pages/Tools/SalesForce/AuthorizationSetupPrologue.aspx");
        if (URLHelper.Url.Scheme == "https" || ValidationHelper.GetBoolean(SettingsHelper.AppSettings["CMSSalesForceHttpCallbackEnabled"], false))
        {
            baseUrl = URLHelper.ResolveUrl("~/CMSModules/ContactManagement/Pages/Tools/SalesForce/AuthorizationSetup.aspx");
        }
    ...
    private Hashtable CreateParameters()
    {
        ...
        parameters.Add("UrlScheme", URLHelper.Url.Scheme);
        parameters.Add("UrlPort", URLHelper.Url.Port);
