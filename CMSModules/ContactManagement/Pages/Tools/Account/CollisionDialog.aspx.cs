...
public partial class CMSModules_ContactManagement_Pages_Tools_Account_CollisionDialog : CMSModalPage
{
    ...
    private void FillTooltipData(Image image, DataTable dt, string fieldName, string fieldValue, FormFieldDataTypeEnum dataType)
    {
        ...
        if (!String.IsNullOrEmpty(fieldValue))
        {
            ...
            {
                image.ToolTip += "<strong>" + ValidationHelper.GetDateTime(fieldValue, DateTimeHelper.ZERO_TIME) + "</strong>";
            }
            else
            {
                // Country
        ...
        else
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                if (fieldName == "AccountStatusID")
                {
                    ...
                    image.ToolTip += "<strong>" + HTMLHelper.HTMLEncode(ResHelper.LocalizeString(status.AccountStatusDisplayName)) + "</strong><br />";
    ...
    private void InitCountry()
    {
        ...
        countrySelector.CountryID = PreselectCountryState(parentAccount.AccountCountryID, table);
        ...
        countrySelector.StateID = PreselectCountryState(parentAccount.AccountStateID, table);
    ...
    {
        mergedAccounts.Tables[0].DefaultView.Sort = sortRule;
        mergedAccounts.Tables[0].DefaultView.RowFilter = rowFilter;
        return mergedAccounts.Tables[0].DefaultView.ToTable(true, groupByColumn);
    }
