...
public partial class CMSModules_ContactManagement_Pages_Tools_Contact_CollisionDialog : CMSModalPage
{
    ...
    private void Initialize()
    {
        if (!DataHelper.DataSourceIsEmpty(mergedContacts) && (parentContact != null))
        {
            if (!RequestHelper.IsPostBack())
            {
                ...
                BindCombobox(cmbContactCompanyName, "ContactCompanyName", parentContact.ContactCompanyName, imgContactCompanyName);
    ...
    {
        // Get grouped dataset
        DataTable dt = SortGroupContactsByColumn(fieldName + " ASC", fieldName + " NOT LIKE ''", fieldName);
    ...
    private void InitLastName()
    {
        ...
        // Insert parent value to first position
        if (!String.IsNullOrEmpty(parentContact.ContactLastName) && !parentContact.ContactLastName.StartsWith(ContactHelper.MERGED) && !cmbContactLastName.DropDownList.Items.Contains(new ListItem(parentContact.ContactLastName)))
        {
            cmbContactLastName.DropDownList.Items.Insert(0, parentContact.ContactLastName);
            cmbContactLastName.SelectedValue = parentContact.ContactLastName;
        }
        ...
        // Display appropriate icon
        DisplayTooltip(imgContactLastName, SortGroupContactsByColumn("ContactLastName ASC", "ContactLastName NOT LIKE ''", "ContactLastName"), "ContactLastName", parentContact.ContactLastName, FormFieldDataTypeEnum.Unknown);
    }
    ...
    private void InitCountrySelector()
    {
        ...
        countrySelector.CountryID = PreselectCountryState(parentContact.ContactCountryID, table);
        ...
        countrySelector.StateID = PreselectCountryState(parentContact.ContactStateID, table);
    ...
    private void InitContactStatus()
    {
        ...
        string statusName = csi == null ? null : HTMLHelper.HTMLEncode(ResHelper.LocalizeString(csi.ContactStatusDisplayName));
...
"));
    ...
    private void Save()
    {
        ...
        parentContact.ContactCompanyName = cmbContactCompanyName.Text.Trim();
        ...
        ContactInfoProvider.SetContactInfo(parentContact, false);
    }
    ...
    private bool ValidateForm()
    {
        ...
        if (!calendarControl.IsValid())
        {
            ...
            ShowError(String.Format(GetString("om.contact.enterbirthday"), showDate.ToString("d")));
            return false;
        }
