...
public partial class CMSModules_Content_CMSDesk_New_NewFile : CMSContentPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!DataClassInfoProvider.IsChildClassAllowed(DocumentManager.ParentNode.GetIntegerValue("NodeClassID"), DataClass.ClassID))
        {
        ...
        if (!CMSContext.CurrentUser.IsAuthorizedToCreateNewDocument(DocumentManager.ParentNode, DataClass.ClassName))
        {
        ...
        if (RequiresDialog)
        {
            SetTitle("CMSModules/CMS_Content/Menu/NewFile.png", GetString("Content.NewFile"), null, null);
        }
    }
    ...
    void DocumentManager_OnSaveData(object sender, DocumentManagerEventArgs e)
    {
        ...
        else
        {
            ...
            DocumentHelper.SaveTemporaryAttachments(node, Guid, CMSContext.CurrentSiteName, DocumentManager.Tree);
        }
