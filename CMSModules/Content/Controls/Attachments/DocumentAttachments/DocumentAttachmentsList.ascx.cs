...
", ClientID, hdnFullPostback.ClientID, hdnPostback.ClientID, ScriptHelper.GetString(GetString("attach.deleteconfirmation")));
            ...
            gridAttachments.AllColumns = SqlHelperClass.MergeColumns(info.ColumnNames).Replace("AttachmentHistoryID", "AttachmentID");
        }
    }
  ...
  return false; 
    ...
    protected DataSet gridAttachments_OnDataReload(string completeWhere, string currentOrder, int currentTopN, string columns, int currentOffset, int currentPageSize, ref int totalRecords)
    {
        if (Node != null)
        {
            ...
            dsAttachments.CultureCode = !string.IsNullOrEmpty(Node.DocumentCulture) ? Node.DocumentCulture : CMSContext.PreferredCultureCode;
    ...
    {
        ImageButton imgClone = (ImageButton)sender;
        if (!Enabled || (DocumentID <= 0))
        {
            imgClone.Style.Add("cursor", "default");
