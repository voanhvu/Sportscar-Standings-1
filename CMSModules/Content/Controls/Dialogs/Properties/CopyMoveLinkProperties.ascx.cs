...
public partial class CMSModules_Content_Controls_Dialogs_Properties_CopyMoveLinkProperties : ContentActionsControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!RequestHelper.IsCallback())
        {
            ...
            MessagesPlaceHolder.WrapperControlClientID = pnlGeneralTab.ClientID;
        ...
        finally
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        ctlAsync.Parameter = "RefreshTree(" + parentId + ", " + parentId + ");SelectNode(" + parentId + ");";
                    }
                }
            }
        }
    ...
    private void HandleNonExistentDocument(string siteName)
    {
        ...
        if (root != null)
        {
            ctlAsync.Parameter = "SelectNode(" + root.NodeID + ");RefreshTree(" + root.NodeID + ", " + root.NodeID + ");";
        }
    }
    ...
    protected void RunAsync(AsyncAction action)
    {
        ...
        {
            case Action.Copy:
                ctlAsync.Parameter = String.Format("{0};{1}", ValidationHelper.GetBoolean(WindowHelper.GetItem(Action.Copy + underlying), false), copyPerm);
                break;
            case Action.Link:
            case Action.LinkDoc:
                ctlAsync.Parameter = String.Format("{0};{1}", ValidationHelper.GetBoolean(WindowHelper.GetItem(Action.Link + underlying), false), copyPerm);
                break;
            case Action.Move:
