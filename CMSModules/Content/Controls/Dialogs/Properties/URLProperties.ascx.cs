...
public partial class CMSModules_Content_Controls_Dialogs_Properties_URLProperties : ItemProperties
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!StopProcessing)
        {
            ...
            btnTxtHidden.Click += new EventHandler(btnTxtHidden_Click);
            btnHiddenSize.Click += new EventHandler(btnHiddenSize_Click);

            if (IsSelectPath || IsRelationship)
            {
                plcUrl.Visible = false;
                plcSizeSelectorHeight.Visible = false;
    ...
    private string EnsureMediaSelector(string url)
    {
        if (ShowSizeControls)
        {
            url = CMSDialogHelper.UpdateUrl(widthHeightElem.Width, widthHeightElem.Height, 0, 0, url, SourceType, true);
        }
    ...
    private void LoadPreview()
    {
        ...
        if (CurrentIsImage)
        {
            ...
            imagePreview.Width = widthHeightElem.Width;
            imagePreview.Height = widthHeightElem.Height;
        }
        else
        {
            string url = txtUrl.Text.Trim();
    ...
    private void DisplaySizeSelector(bool display)
    {
        if (display)
        {
            plcSizeSelectorWidth.Visible = true;
            plcSizeSelectorHeight.Visible = true;
            previewTd.Attributes["rowspan"] = "3";
            previewTd.Attributes["colspan"] = "1";
        }
        else
        {
            plcSizeSelectorWidth.Visible = false;
            plcSizeSelectorHeight.Visible = false;
            previewTd.Attributes["rowspan"] = "1";
            previewTd.Attributes["colspan"] = "3";
        }
    }
    ...
    public override void LoadProperties(Hashtable properties)
    {
        if (properties != null)
        {
            ...
            {
                txtSelectPath.Text = ValidationHelper.GetString(properties[DialogParameters.DOC_NODEALIASPATH], "");
    ...
    private bool DisplaySizeSelector()
    {
        ...
        result = result || (IsMediaSelector && (CurrentIsImage || CurrentIsMedia));
    ...
    public override Hashtable GetItemProperties()
    {
        ...
        if (!(IsRelationship || IsSelectPath))
        {
            bool resolveUrl = !((Config.OutputFormat == OutputFormatEnum.URL) && (Config.SelectableContent == SelectableContentEnum.OnlyMedia));
            ...
            else if (tabImageGeneral.Visible)
            {
                ...
                {
                    imgUrl = URLHelper.UpdateParameterInUrl(imgUrl, "width", widthHeightElem.Width.ToString());
                }
                if ((widthHeightElem.Height != DefaultHeight) && sizeToUrl)
                {
                    imgUrl = URLHelper.UpdateParameterInUrl(imgUrl, "height", widthHeightElem.Height.ToString());
                }
