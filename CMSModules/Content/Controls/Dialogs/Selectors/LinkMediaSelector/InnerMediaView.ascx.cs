...
public partial class CMSModules_Content_Controls_Dialogs_Selectors_LinkMediaSelector_InnerMediaView : CMSUserControl
{
        ...
        get
        {
            ...
            switch (ViewMode)
            {
                case DialogViewModeEnum.TilesView:
                    pager = pagerElemTiles.Pager;
                    pageSize = pagerElemTiles.GetPageSize(20);
                    break;

                case DialogViewModeEnum.ThumbnailsView:
                    pager = pagerElemThumbnails.Pager;
                    pageSize = pagerElemThumbnails.GetPageSize(12);
                    break;

                case DialogViewModeEnum.ListView:
                    pager = gridList.Pager.UniPager;
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        {
            case DialogViewModeEnum.TilesView:
                pagerElemTiles.Pager.CurrentPage = 0;
                break;

            case DialogViewModeEnum.ThumbnailsView:
                pagerElemThumbnails.Pager.CurrentPage = 0;
                break;
        }
...
function RaiseMassAction(drpActionsClientId, drpActionFilesClientId) {
    ...
    /// </summary>
    private List<string> GetTilesThumbsSelectedItems()
    {
    ...
    private void InitializeControlScripts()
    {
        ...
        {
            urlMeta = CMSContext.ResolveDialogUrl("~/CMSModules/AdminControls/Controls/MetaFiles/MetaDataEditor.aspx");
        }
...
function Edit(param) {{
    ...
    private int GetObjectSiteID(string objectType, int objectID)
    {
        if (mObjectSiteID == -1)
        {
            ...
            mObjectSiteID = info != null ? info.Generalized.ObjectSiteID : CMSContext.CurrentSiteID;
        }
