...
public partial class CMSModules_Content_Controls_Dialogs_Selectors_LinkMediaSelector_LinkMediaSelector : LinkMediaSelector
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        mediaView.ShowParentButton = (mediaView.ShowParentButton && (StartingPathNodeID != NodeID) && IsFullListingMode);
    ...
    private string GetSiteWhere()
    {
        ...
        if (!String.IsNullOrEmpty(siteName))
        {
            // Ensure that selected site is always loaded (cancel the "only running" condition)
        ...
        if (nodeId > 0)
        {
            ...
            if (tree != null)
            {
                ...
                using (TreeNode node = tree.SelectSingleNode(nodeId))
                {
                    if (node != null)
                    {
                        return ((node.NodeChildNodesCount == 0) ? TreePathUtils.GetParentPath(node.NodeAliasPath) : node.NodeAliasPath).ToLowerCSafe();
                    }
                }
            }
        }
            ...
            if ((TreeNodeObj != null) && !(Config.OutputFormat == OutputFormatEnum.NodeGUID && node.NodeSiteID != CMSContext.CurrentSiteID))
            {
                ...
                if (si != null)
                {
                    ...
                    if (node.NodeClassName.ToLowerCSafe() != "cms.file")
                    {
                        ...
                        // Check permissions
                        if (CMSContext.CurrentUser.IsAuthorizedPerDocument(node, NodePermissionsEnum.ExploreTree) == AuthorizationResultEnum.Allowed)
                        {
                            ...
                            if (attachmentGUID != Guid.Empty)
                            {
                                ...
                                if (atInfo != null)
                                {
                                    ...
                                    if (CMSDialogHelper.IsItemSelectable(SelectableContent, extension, isContentFile))
                                    {
                                        ...
                                        fileExtension = (isContentFile ? TreePathUtils.GetFilesUrlExtension(SiteName) : TreePathUtils.GetUrlExtension(SiteName));
                                        if (String.IsNullOrEmpty(fileExtension))
                                        {
                                            fileExtension = node.DocumentExtensions;
                                        }
...
if (wopener.parent != null) {
    ...
    private void LoadAttachmentsDataSource(string searchText)
    {
        ...
        string where = String.IsNullOrEmpty(searchText) ? "(AttachmentIsUnsorted = 1)" : SqlHelperClass.AddWhereCondition("(AttachmentIsUnsorted = 1)", String.Format("(AttachmentName LIKE N'%{0}%')", SqlHelperClass.GetSafeQueryString(searchText, false)));
        ...
        else if (AttachmentsAreTemporary)
        {
            ...
            attachments = AttachmentInfoProvider.GetAttachments(where, "AttachmentOrder, AttachmentName", false, topN);
        }
    ...
    private void HandleAttachmentEdit(string argument)
    {
        ...
        if (!string.IsNullOrEmpty(argument))
        {
            ...
            {
                ai = AttachmentInfoProvider.GetAttachmentInfo(attachmentGuid, CMSContext.CurrentSiteName);
            }
            else
            {
    ...
    {
        int topN = mediaView.CurrentTopN;
        string where = !String.IsNullOrEmpty(searchText) ? String.Format("(MetaFileName LIKE N'%{0}%')", SqlHelperClass.GetSafeQueryString(searchText, false)) : string.Empty;

        // Get metafiles
        mediaView.DataSource = MetaFileInfoProvider.GetMetaFiles(Config.MetaFileObjectID, Config.MetaFileObjectType, Config.MetaFileCategory, where, null, "MetaFileID,MetaFileObjectType,MetaFileObjectID,MetaFileGroupName,MetaFileName,MetaFileExtension,MetaFileSize,MetaFileMimeType,MetaFileImageWidth,MetaFileImageHeight,MetaFileGUID,MetaFileLastModified,MetaFileSiteID,MetaFileTitle,MetaFileDescription", topN);
    }
    ...
    private string CheckMetaFilePermissions()
    {
        ...
        if (!UserInfoProvider.IsAuthorizedPerObject(Config.MetaFileObjectType, Config.MetaFileObjectID, PermissionsEnum.Modify, SiteName, CMSContext.CurrentUser))
        {
