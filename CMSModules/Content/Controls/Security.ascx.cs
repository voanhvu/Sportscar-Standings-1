...
public partial class CMSModules_Content_Controls_Security : CMSUserControl
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if (!IsLiveSite && DocumentManager.RegisterSaveChangesScript)
        {
            ...
            script.Append(@"
var select = $j('#", lstOperators.ClientID, @"');
select.change(function(){
    if(!CheckChanges()) {
        select.val($j.data(select, 'current'));
        return false;
    }
    $j.data(select, 'current', select.val());
});");
    ...
    private void DisableForm()
    {
        ...
        if (DisplaySecurityMessage)
        {
            ...
            lblPermission.Text = String.Format(GetString("cmsdesk.notauthorizedtoeditdocumentpermissions"), Node.NodeAliasPath);
        }
    }
