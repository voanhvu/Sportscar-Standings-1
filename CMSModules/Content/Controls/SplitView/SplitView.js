...
function FSP_BindScrollElements() {
    ...
    if (FSP_scrollableDiv != null) {
        if (FSP_bindElement1 == null) {
            FSP_bindElement1 = $j(FSP_frameF1.contentWindow.document.getElementById(FSP_scrollableDiv));
        }
        if (FSP_bindElement2 == null) {
            FSP_bindElement2 = $j(FSP_frameF2.contentWindow.document.getElementById(FSP_scrollableDiv));
        }
    }
    ...
    if ($j.browser.mozilla && parseInt($j.browser.version) > 13) {
...
function SetSplitViewFrameUrl(newUrl) {
    // Get frame2 window
