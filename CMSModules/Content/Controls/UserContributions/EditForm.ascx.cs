...
public partial class CMSModules_Content_Controls_UserContributions_EditForm : CMSUserControl
{
    ...
    private FormModeEnum formModebeforeSave = FormModeEnum.Update;
                    ...
                    else
                    {
                        ...
                        if (NewDocument && (ClassID <= 0))
                        {
                            ...
                            if (parentNode != null)
                            {
                                ...
                                DataSet ds = DataClassInfoProvider.GetAllowedChildClasses(ValidationHelper.GetInteger(parentNode.GetValue("NodeClassID"), 0), ValidationHelper.GetInteger(SiteInfoProvider.GetSiteInfo(SiteName).SiteID, 0), "ClassName, ClassDisplayName, ClassID", -1);
    ...
    protected override void OnPreRender(EventArgs e)
    {
        if (Visible)
        {
            if (pnlEdit.Visible)
            {
                ...
                var htmlControls = formElem.BasicForm.FormInformation.GetFields(FormFieldControlTypeEnum.HtmlAreaControl);
                if (htmlControls.Any())
                {
    ...
    private void LogInsertActivity(TreeNode node)
    {
        ...
        Activity activity = new ActivityUserContributionInsert(node, node.GetDocumentName(), CMSContext.ActivityEnvironmentVariables);
        activity.Log();
    }
    ...
    private void LogUpdateActivity(TreeNode node)
    {
        ...
        Activity activity = new ActivityUserContributionUpdate(node, node.GetDocumentName(), CMSContext.ActivityEnvironmentVariables);
        activity.Log();
    }
