...
public partial class CMSModules_Content_Controls_Versions : CMSUserControl
{
    ...
    private void ReloadData()
    {
        ...
        else if (useCheckInCheckOut || (versionsElem.CheckedOutByUserID != 0))
        {
            ...
            {
                lblCheckInfo.Text = GetString("VersionsProperties.CheckOut");
                lblWorkflow.Text = GetString("VersionsProperties.InfoCheckedIn");
                DisableForm();
                btnCheckout.Visible = true;
            ...
            else
            {
                ...
                if (versionsElem.CheckedOutByUserID == CMSContext.CurrentUser.UserID)
                {
                    ...
                    lblWorkflow.Text = GetString("VersionsProperties.InfoCheckedOut");
                }
                else
                {
    ...
    protected void btnCheckin_Click(object sender, EventArgs e)
    {
        ...
        {
            // Check permissions
            if (!WorkflowManager.CheckStepPermissions(Node, WorkflowActionEnum.Approve) || ((versionsElem.CheckedOutByUserID != CMSContext.CurrentUser.UserID) && !versionsElem.CanCheckIn))
            {
                DisableForm();
                return;
            }
