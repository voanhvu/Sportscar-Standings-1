...
public partial class CMSModules_CssStylesheets_Pages_CssStylesheet_New : CMSModalPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        txtText.FullScreenParentElementID = "divContent";
        ...
        else
        {
            ...
            if (CurrentSite != null)
            {
                chkAssign.Text = string.Concat(GetString("General.AssignWithWebSite"), " ", HTMLHelper.HTMLEncode(CurrentSite.DisplayName));
    ...
    private void ProcessDialog(int cssStylesheetId, bool closeOnSave)
    {
        ...
        string script =
            string.Format(@"if (wopener) {{ wopener.US_SelectNewValue_{0}('{1}'); }}",
        ...
        else
        {
            ...
            script += string.Format(@"window.name = {0};
                    	                  window.open('CssStylesheet_General.aspx?cssstylesheetid={1}&saved=1&editonlycode=true&selectorid='+{0}+'&siteid={3}&hash={2}{4}',window.name);",
                                    ScriptHelper.GetString(selector),
                                    cssStylesheetId,
                                    QueryHelper.GetHash("?editonlycode=true"),
                                    siteId,
                                    aliasPath != String.Empty ? "&aliaspath=" + aliasPath : ""
                                    );
        }
