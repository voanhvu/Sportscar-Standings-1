...
public partial class CMSModules_CustomTables_Tools_CustomTable_Import : CMSCustomTablesToolsPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        //if (!IsPostBack)
        {
            ...
            ButtonGet.OnClientClick = "$j('.import_Loading').show(); return true;";
            ButtonExport.OnClientClick = ButtonGet.OnClientClick;
   ...
   ////////////////////////////////////////////////////////////////////////////////////
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        ...
        if (URLHelper.IsPostback() && true)
        {
            ...
            combinationJSList.Append("$j('.import_Loading').hide();");
            // Register the JS arrays and current variants
            ScriptHelper.RegisterStartupScript(this, typeof(string), "combinationJSList_Loading", ScriptHelper.GetScript(combinationJSList.ToString()));
            pnlUpdate.Update();
        }
    ...
    protected string default_value(string name,out string fieldname_excel)
    {
        ...
        if (!DataHelper.DataSourceIsEmpty(ds_excel))
        {
            ...
            {
                f_ex = ds_excel.Tables[0].Rows[0][i].ToString();
                if (f_ex[0].ToString() == "'") f_ex = f_ex.Substring(1, f_ex.Length-1);
                if (ReturnCaptionValue(name) == f_ex || name == f_ex) 
                {
