...
public partial class CMSModules_DocumentTypes_Controls_HierarchicalTransformations_List : CMSAdminEditControl
{
    ...
    protected void ugTransformations_OnAction(string actionName, object actionArgument)
    {
        if (actionName == "edit")
        {
            ...
            URLHelper.Redirect(String.Format("HierarchicalTransformations_Transformations_Edit.aspx?guid={0}&transid={1}&templatetype={2}&editonlycode={3}&tabmode={4}&instanceguid={5}&aliaspath={6}{7}",
                actionArgument, TransInfo.TransformationID, TemplateType, mDialogMode, QueryHelper.GetInteger("tabmode", 0), QueryHelper.GetGuid("instanceguid", Guid.Empty), QueryHelper.GetString("aliaspath", ""), isManager));
        }
