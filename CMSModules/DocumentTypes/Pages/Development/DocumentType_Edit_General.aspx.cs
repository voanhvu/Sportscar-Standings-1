...
public partial class CMSModules_DocumentTypes_Pages_Development_DocumentType_Edit_General : SiteManagerPage
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        // Large icon
        imageUrl = GetDocumentTypeIconUrl(className, "48x48");
        imageUrl = URLHelper.AddParameterToUrl(imageUrl, "chset", Guid.NewGuid().ToString());
        ...
        ltlScript.Text += ScriptHelper.GetScript("function RefreshIcons() { " + ClientScript.GetPostBackEventReference(btnHidden, "") + " }");

        this.plcRootTemplate.Visible = this.chkTemplateSelection.Checked;
        //this.plcDefaultTemplate.Visible = !this.chkTemplateSelection.Checked;
    }
    ...
    protected void LoadData()
    {
        ...
        categorySelector.SelectedCategoryID = classInfo.ClassPageTemplateCategoryID;
    ...
    private static string[] ClassNameParser(string className)
    {
        ...
        int lastDot = className.LastIndexOfCSafe('.');
    ...
    {
        string sourceFile = UIHelper.GetDocumentTypeIconPath(this, className, iconSet, true, true);
        string targetFile = UIHelper.GetDocumentTypeIconPath(this, newClassName, iconSet, false, true);
        string defaultFile = UIHelper.GetDocumentTypeIconPath(this, "default", iconSet, false, true);
            ...
            if (classNameChanged)
            {
                try
                {
                    ...
                    else
                    {
                        ...
                        if (!isCoupled)
                        {
                            sourceFile = UIHelper.GetDocumentTypeIconPath(this, "defaultcontainer", iconSet, false, true);
                            sourceFile = Server.MapPath(sourceFile);
                        }
                        if (File.Exists(sourceFile))
                        {
    ...
    {
        if (chkTemplateSelection.Checked)
        {
