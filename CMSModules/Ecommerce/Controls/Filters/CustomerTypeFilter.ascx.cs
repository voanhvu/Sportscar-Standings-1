    ...
    public string GetWhereCondition()
    {
        switch (ddlCustomerType.SelectedValue)
        {
            ...
            case CUSTOMERS_ANONYMOUS:
                return "(CustomerUserID IS NULL) AND CustomerSiteID = " + CMSContext.CurrentSiteID;

            case CUSTOMERS_REGISTERED:
                return string.Format("(CustomerUserID IS NOT NULL) AND (CustomerUserID IN (SELECT UserID FROM CMS_UserSite WHERE SiteID = {0}))", CMSContext.CurrentSiteID);
        }
