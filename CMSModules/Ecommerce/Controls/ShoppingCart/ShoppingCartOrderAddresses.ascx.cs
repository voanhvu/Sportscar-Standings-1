...
public partial class CMSModules_Ecommerce_Controls_ShoppingCart_ShoppingCartOrderAddresses : ShoppingCartStep
{
    ...
    protected void LoadBillingSelectedValue()
    {
        try
        {
            ...
            else if (ShoppingCart.ShoppingCartBillingAddressID > 0)
            {
                try
                {
                    drpBillingAddr.SelectedValue = ShoppingCart.ShoppingCartBillingAddressID.ToString();
                }
                catch
                {
                }
    ...
    protected void LoadShippingSelectedValue()
    {
        try
        {
            ...
            else if ((ShoppingCart.ShoppingCartShippingAddressID > 0) && (ShoppingCart.ShoppingCartShippingAddressID != ShoppingCart.ShoppingCartBillingAddressID))
            {
                try
                {
                    drpShippingAddr.SelectedValue = ShoppingCart.ShoppingCartShippingAddressID.ToString();
    ...
    protected void LoadCompanySelectedValue()
    {
        try
        {
            ...
            else if ((ShoppingCart.ShoppingCartCompanyAddressID > 0) && (ShoppingCart.ShoppingCartCompanyAddressID != ShoppingCart.ShoppingCartBillingAddressID))
            {
                try
                {
                    drpCompanyAddress.SelectedValue = ShoppingCart.ShoppingCartCompanyAddressID.ToString();
    ...
    public override bool ProcessStep()
    {
        ...
        if (mCustomerId > 0)
        {
            ...
            if (chkCompanyAddress.Checked)
            {
                ...
                // Save address and set it's ID to ShoppingCartInfoObj
                AddressInfoProvider.SetAddressInfo(ai);
                ShoppingCart.ShoppingCartCompanyAddressID = ai.AddressID;
            }
            ...
            else
            {
                ...
                {
                    ShoppingCart.ShoppingCartCompanyAddressID = ShoppingCart.ShoppingCartBillingAddressID;
                }
                else
                {
                    ShoppingCart.ShoppingCartCompanyAddressID = 0;
                }
            }
