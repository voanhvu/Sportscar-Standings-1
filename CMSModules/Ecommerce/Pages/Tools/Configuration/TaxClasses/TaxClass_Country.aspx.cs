...
[Security(Resource = "CMS.Ecommerce", UIElements = "Configuration.TaxClasses.Countries")]
...
function switchCurrency(isFlatValue, labelId){{
  ...
  {{
    ...
    protected void Save()
    {
        ...
        for (int i = 0; i < GridViewCountries.Rows.Count; i++)
        {
            Label lblCountryId = (Label)GridViewCountries.Rows[i].Cells[3].Controls[0];
            ...
            if (countryId > 0)
            {
                ...
                if ((mChangedTextBoxes[txtValue.ID] != null) || (mChangedCheckBoxes[chkIsFlat.ID]) != null)
                {
                    ...
                    {
                        TaxClassCountryInfoProvider.RemoveCountryTaxValue(countryId, mTaxClassId);
                    ...
                    else
                    {
                        ...
                        {
                            countryInfo = TaxClassCountryInfoProvider.GetTaxClassCountryInfo(countryId, mTaxClassId);
