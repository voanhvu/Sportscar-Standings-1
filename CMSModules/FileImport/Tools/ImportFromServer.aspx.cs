...
public partial class CMSModules_FileImport_Tools_ImportFromServer : CMSFileImportPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!RequestHelper.IsCallback())
        {
            ...
            {
                cultureSelector.Value = CMSContext.PreferredCultureCode;
        ...
        else
        {
            if (gridImport.SelectedItems.Count > 0)
            {
                ...
                if (!String.IsNullOrEmpty(path))
                {
                    ...
                    ctlAsync.Parameter = new object[4] { gridImport.SelectedItems.ToArray(), path, cultureSelector.Value, CMSContext.CurrentUser };
