...
<asp:Panel runat="server" ID="pnlSubscriptions" Visible="false">
    ...
    <cms:UniGrid ID="forumSubscriptions" runat="server" FilterLimit="10" ExportFileName="forum_subscription">
        <GridActions>
            <ug:Action Name="ForumUnsubscribe" Caption="Unsubscribe" Icon="Delete.png" Confirmation="$forum.confirmunsubscribe$" />
        </GridActions>
        <GridColumns>
            <ug:Column Source="ForumDisplayName" Caption="$unigrid.forums.columns.forumname$" Width="100%"
    ...
    <cms:UniGrid ID="postSubscription" runat="server" FilterLimit="10" ExportFileName="forumpost_subscription">
        <GridActions Parameters="PostID">
            <ug:Action Name="PostUnsubscribe" Caption="Unsubscribe" Icon="Delete.png" Confirmation="$forumpost.confirmunsubscribe$" />
            <ug:Action Name="View" Caption="$forumpost.postpreview$" Icon="View.png" OnClick="var ret = true; if (window.ForumPostUnsubscribe) {ret = window.ForumPostUnsubscribe({0});} if (ret == false) {return false;} " />
        </GridActions>
        <GridColumns>
            ...
            <ug:Column Source="PostSubject" Caption="$Forums.PostListing.PostSubject$" Width="100%"
                Wrap="false" Localize="true">
                <Filter Type="text" />
            </ug:Column>
        </GridColumns>
...
<asp:Panel runat="server" ID="pnlNoSubscription" Visible="false">
    <cms:LocalizedLabel ID="forumsNoData" runat="server" ResourceString="forumsubscripitons.userhasnosubscriptions" />
</asp:Panel>
