...
public partial class CMSModules_Forums_Controls_Subscriptions_UserSubscriptionList : CMSAdminControl
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        else
        {
            ...
            function ForumPostUnsubscribe(id) {
                var url = '" + URLHelper.ResolveUrl(dialogUrl) + @"';
        ...
        pnlSeparator.Visible = (forumSubscriptions.RowsCount == 0) ? false : true;
    }
    ...
    private void SetupControls()
    {
        if (UserID > 0)
        {
            ...
            forumSubscriptions.OnDataReload += new OnDataReloadEventHandler(forumSubscriptions_OnDataReload);
            forumSubscriptions.OnAction += new OnActionEventHandler(forumSubscriptions_OnAction);
            ...
            postSubscription.OnDataReload += new OnDataReloadEventHandler(postSubscription_OnDataReload);
            postSubscription.OnAction += new OnActionEventHandler(forumSubscriptions_OnAction);
            postSubscription.OnExternalDataBound += new OnExternalDataBoundEventHandler(postSubscription_OnExternalDataBound);
