...
public partial class CMSModules_Groups_Controls_Members_MemberEdit : CMSAdminEditControl
{
    ...
    private bool currentRolesLoaded = false;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            userSelector.SiteID = CMSContext.CurrentSite.SiteID;
        }
    ...
    public override void ReloadData()
    {
        ...
        if ((MemberID > 0))
        {
            gmi = GroupMemberInfoProvider.GetGroupMemberInfo(MemberID);
        }
    ...
    public bool ApproveMember()
    {
        ...
        EnsureMember();
        if ((gmi != null) && (CMSContext.CurrentUser != null))
        {
            // Set properties
            ...
            lblMemberApproved.Text = GetApprovalInfoText(gmi.MemberApprovedWhen, gmi.MemberApprovedByUserID);
            lblMemberRejected.Text = GetApprovalInfoText(gmi.MemberRejectedWhen, gmi.MemberApprovedByUserID);
            return true;
        }
        return false;
    }
    ...
    private string CreateWhereCondition()
    {
        ...
        {
            // Member
            where = "(RoleGroupID = " + gmi.MemberGroupID + ")";
        }
        else
        {
            // Group
    ...
    private void InitializeForm()
    {
        ...
        pnlRoles.GroupingText = GetString("group.member.memberinroles");

        if (newItem)
        {
            pnlRoles.GroupingText = GetString("group.member.addmemberinroles");
        }
    ...
    private void HandleExistingMember(GroupMemberInfo gmi)
    {
        if (gmi != null)
        {
            ...
            txtComment.Text = gmi.MemberComment;
            lblMemberApproved.Text = GetApprovalInfoText(gmi.MemberApprovedWhen, gmi.MemberApprovedByUserID);
            lblMemberRejected.Text = GetApprovalInfoText(gmi.MemberRejectedWhen, gmi.MemberApprovedByUserID);
            lblMemberJoined.Text = CMSContext.ConvertDateTime(gmi.MemberJoined, this).ToString();
        }
    }
