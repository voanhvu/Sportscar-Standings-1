...
public partial class CMSModules_Groups_Controls_Security_GroupSecurity : CMSAdminEditControl, IPostBackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (resGroups != null)
        {
            ...
            string where = "RoleGroupID=" + GroupID.ToString() + "AND PermissionDisplayInMatrix = 0";
            ...
            {
                if (!CMSContext.CurrentUser.IsAuthorizedPerResource("cms.groups", PERMISSION_MANAGE))
                {
                    Enabled = false;
                    gridMatrix.Enabled = false;
                    ShowError(String.Format(ResHelper.GetString("CMSSiteManager.AccessDeniedOnPermissionName"), "Manage"));
                }
            }
        }
    }
