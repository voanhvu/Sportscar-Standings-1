...
public partial class CMSModules_IntranetPortal_Controls_EmployeesFilterControl : CMSAbstractBaseFilterControl
{
    ...
    protected string GenerateWhereCondition(string searchPhrase)
    {
        ...
        if (drpDepartment.SelectedValue != String.Empty)
        {
            ...
            whereCondition = SqlHelperClass.AddWhereCondition(whereCondition, whereDepartment);
        }
    ...
    {
        DataSet dsDepartments = TreeProvider.SelectNodes(CMSContext.CurrentSiteName, "/%", null, true, DEPARTMENT_DOC_TYPE, null, null, -1, true, 0, TreeProvider.SELECTNODES_REQUIRED_COLUMNS + ",DocumentName");
