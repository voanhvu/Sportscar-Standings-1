...
public partial class CMSModules_MediaLibrary_Controls_Dialogs_MediaView : MediaView
{
    ...
    /// <summary>
    /// Gets or sets the value which determined whether to show the Azure button or not.
    /// </summary>
    public bool ShowPrepareForImportButton
    {
        get;
        set;
    }
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        if (ShowParentButton)
        {
            ...
            spnParent.InnerText = GetString("dialogs.mediaview.parentfolder");
            btnParent.Attributes["onclick"] = "SetAction('parentselect', ''); RaiseHiddenPostBack(); return false;";
        }

        // Azure import prepare button
        if (ShowPrepareForImportButton)
        {
            plcExtStorage.Visible = true;
            imgExtStoragePrepare.ImageUrl = ResolveUrl(GetImageUrl("CMSModules/CMS_Content/Dialogs/importprepare.png", IsLiveSite));
            spnExtStoragePrepare.InnerText = GetString("dialogs.mediaview.azureprepare");
            lnkExtStoragePrepare.Attributes["onclick"] = "ShowIndicator(true, true); SetAction('externalstorageprepare', ''); RaiseHiddenPostBack(); return false;";
    ...
    private void SetupControls()
    {
        ...
        string folderPath = MediaLibraryInfoProvider.GetMediaLibraryFolderPath(LibraryInfo);

        //Ensure azure button
        ShowPrepareForImportButton = StorageHelper.IsExternalStorage(folderPath) && !IsCopyMoveLinkDialog;
