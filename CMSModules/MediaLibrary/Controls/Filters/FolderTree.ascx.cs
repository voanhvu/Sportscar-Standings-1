...
public partial class CMSModules_MediaLibrary_Controls_Filters_FolderTree : FolderTree
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (ValidationHelper.GetString(Request.Params["__EVENTTARGET"], String.Empty).StartsWithCSafe(folderTree.UniqueID))
        {
            // Update information on currently selected path
            string selectedPath = ValidationHelper.GetString(Request.Params["__EVENTARGUMENT"], String.Empty).ToLowerCSafe();
            if (selectedPath != "")
            {
                // Remove library root
            ...
            if (GetFileID() > 0)
            {
                string url = URLHelper.RemoveParameterFromUrl(URLHelper.CurrentURL, FileIDQueryStringKey);
                if (FilterMethod == 0)
                {
