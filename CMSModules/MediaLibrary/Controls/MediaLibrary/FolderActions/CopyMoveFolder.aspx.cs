    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsLoad)
        {
            if (AllFiles || String.IsNullOrEmpty(Files))
            {
                if (AllFiles)
                {
                    lblFilesToCopy.ResourceString = "media.folder.filestoall" + Action.ToLowerCSafe();
                }
                else
                {
                    lblFilesToCopy.ResourceString = "media.folder.folderto" + Action.ToLowerCSafe();
                }
            ...
            {
                lblFilesToCopy.ResourceString = "media.folder.filesto" + Action.ToLowerCSafe();
    ...
    private void PerformAction(object parameter)
    {
        ...
        if (LibraryInfo != null)
        {
            ...
            if (String.IsNullOrEmpty(Files) && !AllFiles)
            {
                ...
                if ((Action.ToLowerCSafe() == "move") && (newPath == FolderPath))
                {
                ...
                if (String.IsNullOrEmpty(Files) && (Action.ToLowerCSafe() == "move") && newPath.StartsWithCSafe(DirectoryHelper.EnsurePathBackSlash(FolderPath))
            ...
            if ((String.IsNullOrEmpty(Files) && !AllFiles) && (Action.ToLowerCSafe() == "move") && newPath.StartsWithCSafe(DirectoryHelper.EnsurePathBackSlash(FolderPath)))
            {
            ...
            if ((!String.IsNullOrEmpty(Files) || AllFiles) && (Action.ToLowerCSafe() == "move") && (newPath.TrimEnd('\\') == FolderPath.TrimEnd('\\')))
            {
...
if (topWin) {
    ...
    CloseDialog();
            ...
            if (String.IsNullOrEmpty(Files) && !mAllFiles)
            {
                ...
                catch (UnauthorizedAccessException ex)
                {
                    ...
                    ev.LogEvent("MediaFolder", Action, ex);
                    AddLog(CurrentError);
                    return;
                }
            ...
            else
            {
                ...
                if (!mAllFiles)
                {
                    ...
                    catch (UnauthorizedAccessException ex)
                    {
                        ...
                        EventLogProvider ev = new EventLogProvider();
                        ev.LogEvent("MediaFile", Action, ex);
                        AddLog(CurrentError);
                        return;
                    }
                    catch (ThreadAbortException ex)
    ...
    private bool CheckPermissions()
    {
        ...
        if (String.IsNullOrEmpty(Files) && !mAllFiles)
        {
            if (Action.ToLowerCSafe().Trim() == "copy")
            {
    ...
    {
        CurrentInfo = (Action.ToLowerCSafe() == "copy") ? GetString("media.copy.canceled") : GetString("media.move.canceled");
        AddLog(CurrentInfo);
