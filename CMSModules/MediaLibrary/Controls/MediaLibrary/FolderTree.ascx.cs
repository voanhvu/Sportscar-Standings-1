...
public partial class CMSModules_MediaLibrary_Controls_MediaLibrary_FolderTree : CMSAdminControl
{
    ...
    private int MaxSubFolders
    {
        get
        {
            if (mMaxSubFolders < 0)
            {
                mMaxSubFolders = SettingsKeyProvider.GetIntValue(CMSContext.CurrentSiteName + ".CMSMediaLibraryMaxSubFolders");
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!StopProcessing)
        {
            ...
            treeElem.SelectedNodeChanged += new EventHandler(treeElem_SelectedNodeChanged);
    ...
    private void BindTreeView(string dirPath, TreeNode parentNode, bool isRoot)
    {
        ...
        {
            string folderImageTag = (DisplayFolderIcon ? "<img src=\"" + GetImageUrl("Design/Controls/Tree/folder.gif") + "\" alt=\"\" style=\"border:0px;vertical-align:middle;\" />" : "");
            ...
            if (dirs != null)
            {
                ...
                {
                    if (!dir.EndsWithCSafe(hidenFolder))
                    {
                        // Add node
    ...
    private TreeNode CreateNode(string nodeText, string nodeValue, TreeNode parrentNode, int childCount, int index)
    {
        ...
        if (!String.IsNullOrEmpty(CustomSelectFunction))
        {
            ...
            if (parrentNode != null)
            {
                val = parrentNode.ValuePath.ToLowerCSafe() + treeElem.PathSeparator + nodeValue.ToLowerCSafe();
            }
            ...
            {
                output += "onClick=\" document.getElementById('" + hdnPath.ClientID + "').value = '" + parrentNode.ValuePath.Replace("\\", "\\\\") + "'; ";
                output += CustomClickForMoreFunction.Replace("##NODEVALUE##", parrentNode.ValuePath.Replace('\\', '|')) + " return false;\" ";
            }
