...
public partial class CMSModules_MediaLibrary_Controls_MediaLibrary_MediaLibrary : CMSAdminItemsControl
{
    ...
    /// <summary>
    /// Percent escape character used in LIKE statement of where condition.
    /// </summary>
    ...
    private string CurrentAction
    {
        get
        {
            ...
            {
                mCurrentAction = hdnAction.Value.Trim().ToLowerCSafe();
            }
    ...
    private string GetCurrentPath()
    {
        ...
        {
            path = LibraryPath + "/" + MediaLibraryHelper.EnsurePath(LastFolderPath) + "/";
        }
            ...
            if (err == string.Empty)
            {
                ...
                if (!IsCopyMoveLinkDialog)
                {
                    ...
                    if (DisplayOnlyImportedFiles)
                    {
                        ...
                        normFolderPath = SqlHelperClass.GetSafeQueryString(normFolderPath, false);
    ...
    /// <param name="expression">Expression used to sort the data</param>
    ...
    private void HandleExternalStoragePrepare()
    {
        ...
        ScriptHelper.RegisterStartupScript(Page, typeof(string), "Hide", ScriptHelper.GetScript("HideIndicator(false);"));
    ...
    private void HandleBulkAction(string parameters)
    {
        ...
        if (tokens.Length == 2 && LibraryInfo != null)
        {
            ...
            {
                ShowError(GetString("Media.Error.NoFilesSelected"));
            }
            else
            {
                switch (action.ToLowerInvariant())
                {
                    ...
                    case "import":
                        ...
                        break;
                    default:
                        ShowError(GetString("media.mass.noaction"));
                        break;
                }
            }
        }
    ...
    protected string multipleImport_GetItemUrl(string fileName)
    {
        ...
        {
            string path = GetCurrentPath();
            string where = String.Format("FileName LIKE '{0}'", DataHelper.EscapeLikeQueryPatterns(fileName, true, true, true).Replace("'", "''"));
            ...
            if (!DataHelper.DataSourceIsEmpty(fileData))
            {
                ...
                if (external && fileUrl.StartsWith(URLHelper.ApplicationPath))
                {
    ...
    private void BulkDelete(IList<string> fileNames, bool includeAllFiles)
    {
        ...
        {
            ShowError(GetString("media.delete.failed"), exception.Message, null);
        }
        finally
        {
            mediaView.ResetListSelection();
