...
public partial class CMSModules_Membership_Controls_UnlockUserAccount : CMSUserControl
{
    ...
    public UserInfo UserAccount
    {
        get
        {
            ...
            {
                mUserAccount = UserInfoProvider.GetUserInfoWithSettings("UserInvalidLogOnAttemptsHash = '" + SqlHelperClass.GetSafeQueryString(mInvalidAttemptsHash) + "'");
            }
    ...
    {        
        btnConfirm.CssClass = DataHelper.GetNotEmpty(UnlockButtonCssClass, "LongButton");
            ...
            if (!checkOnly)
            {
                ...
                if (string.IsNullOrEmpty(RedirectionURL))
                {
                    ...
                    else
                    {
                        ...
                        string url = URLHelper.ResolveUrl(SettingsKeyProvider.GetStringValue(CMSContext.CurrentSiteName + ".CMSSecuredAreasLogonPage"));
