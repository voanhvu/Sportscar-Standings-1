...
public partial class CMSModules_Membership_Pages_Users_User_Edit_General : CMSUsersPage
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (currentUser.IsGlobalAdministrator && (ui != null) && (ui.UserID != currentUser.UserID) && !ui.IsPublic())
        {
    ...
    protected void Page_PreRender(object sender, EventArgs e)
    {
        if (ui != null)
        {
            // Only admin who have currently access to site manager can disable access to site manager to others
            ...
            if (!ui.Enabled)
            {
                ...
                ShowWarning(GetString("Administration-User_List.AccountLocked"), description, description);
            }
    ...
    protected void LoadPasswordExpiration(UserInfo ui)
    {
        ...
        if (!AuthenticationHelper.IsPasswordExpirationEnabled(CMSContext.CurrentSiteName, out expDays))
        {
        ...
        {
            int dayDiff = (DateTime.Now - ui.UserPasswordLastChanged).Days - expDays;
            if (dayDiff >= 0)
            {
    ...
    protected void btnResetLogonAttempts_Click(object sender, EventArgs e)
    {
        ...
        if (unlocked)
        {
            ShowConfirmation(GetString("Administration-User.InvalidLogonAttemptsResetUnlock"));
        }
        else
        {
            ShowConfirmation(GetString("Administration-User.InvalidLogonAttemptsReset"));
        }
