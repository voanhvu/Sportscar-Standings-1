...
public partial class CMSModules_MessageBoards_Controls_Boards_BoardEdit : CMSAdminEditControl
{
    ...
    private void SetupControls()
    {
        ...
        lblBoardOwnerText.Attributes.Add("style", "font-weight: bold;");
        ...
        chkInheritOptInURL.Attributes.Add("onclick", "check('" + txtOptInURL.PathTextBox.ClientID + "', this,'" + ValidationHelper.GetString(SettingsKeyProvider.GetStringValue(CMSContext.CurrentSiteName + ".CMSBoardSubsriptionApprovalURL"), "") + "');ChangeState_" + txtOptInURL.ClientID + "(!this.checked);");

        chkEnableOptIn.NotSetChoice.Text = chkSendOptInConfirmation.NotSetChoice.Text = GetString("general.sitesettings") + " (##DEFAULT##)";
        chkEnableOptIn.SetDefaultValue(BoardInfoProvider.EnableDoubleOptIn(CMSContext.CurrentSiteName));
        chkSendOptInConfirmation.SetDefaultValue(BoardInfoProvider.SendOptInConfirmation(CMSContext.CurrentSiteName));
