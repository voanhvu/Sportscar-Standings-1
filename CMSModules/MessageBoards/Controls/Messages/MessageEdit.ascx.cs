...
public partial class CMSModules_MessageBoards_Controls_Messages_MessageEdit : CMSAdminEditControl
{
    ...
    private void SetupControls()
    {
        ...
        if (!AdvancedMode && BoardProperties.EnableContentRating)
        {
    ...
    private string ValidateForm()
    {
        ...
        // Check security code
        if (string.IsNullOrEmpty(errorMessage) && BoardProperties.BoardUseCaptcha)
        {
    ...
    {
        Activity activity = new ActivityMessageBoardComment(bmi, bi, CMSContext.CurrentDocument, CMSContext.ActivityEnvironmentVariables);
        activity.Log();
    }
