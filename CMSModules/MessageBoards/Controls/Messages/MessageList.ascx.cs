...
public partial class CMSModules_MessageBoards_Controls_Messages_MessageList : CMSAdminListControl
{
    ...
    public bool ShowPermissionMessage
    {
        get
        {
            return mShowPermissionMessage;
        }
        set
        {
            mShowPermissionMessage = value;
        }
    }
    ...
    private int SelectedSiteID
    {
        get
        {
            ...
            int val = ValidationHelper.GetInteger((postbackControl == btnFilter) ? siteSelector.Value : ViewState["SelectedSiteID"], CMSContext.CurrentSiteID);
    ...
    private void SetupControls()
    {
        ...
        rowActions.Visible = AllowMassActions;
    ...
    private string GetFilterWhereCondition()
    {
        ...
        if (SelectedSiteID > 0)
        {
            where = SqlHelperClass.AddWhereCondition(where, "BoardSiteID = " + SelectedSiteID);
        }
        ...
        {
            case "yes":
                where = SqlHelperClass.AddWhereCondition(where, "MessageApproved = 1");
                break;

            case "no":
                where = SqlHelperClass.AddWhereCondition(where, "MessageApproved = 0");
                break;
        }
        ...
        {
            case "yes":
                where = SqlHelperClass.AddWhereCondition(where, "MessageIsSpam = 1");
                break;

            case "no":
                where = SqlHelperClass.AddWhereCondition(where, "MessageIsSpam = 0");
                break;
        }

        if (BoardID > 0)
        {
            where = SqlHelperClass.AddWhereCondition(where, "MessageBoardID = " + mBoardId.ToString());
        }
        else
        {
            // Board dropdown list
            ...
            {
                where = SqlHelperClass.AddWhereCondition(where, "MessageBoardID = " + SelectedBoardID);
            }
            else if (GroupID > 0)
            {
                where = SqlHelperClass.AddWhereCondition(where, "BoardGroupID =" + GroupID);
            }
            else
            {
                where = SqlHelperClass.AddWhereCondition(where, "BoardGroupID IS NULL");
            }
        }

        if (txtUserName.Text.Trim() != "")
        {
            where = SqlHelperClass.AddWhereCondition(where, "MessageUserName LIKE '%" + txtUserName.Text.Trim().Replace("'", "''") + "%'");
        }

        if (txtMessage.Text.Trim() != "")
        {
            where = SqlHelperClass.AddWhereCondition(where, "MessageText LIKE '%" + txtMessage.Text.Trim().Replace("'", "''") + "%'");
        }
    ...
    private string GetWhereCondition()
    {
        ...
        if (!isAuthorized && (!(CMSContext.CurrentUser.IsAuthorizedPerResource("CMS.MessageBoards", "Modify") || CMSContext.CurrentUser.IsGroupAdministrator(GroupID))))
        {
            where = SqlHelperClass.AddWhereCondition(where, "BoardID IN (SELECT BoardID FROM Board_Moderator WHERE Board_Moderator.UserID = " + CMSContext.CurrentUser.UserID + " )");
        }

        // Group restriction
        if (GroupID > 0)
        {
            where = SqlHelperClass.AddWhereCondition(where, "BoardGroupID =" + mGroupId);
        }
        else
        {
            where = SqlHelperClass.AddWhereCondition(where, "(BoardGroupID = 0 OR BoardGroupID IS NULL)");
        }
    ...
    protected void btnFilter_Click(object sender, EventArgs e)
    {
        ...
        SelectedSiteID = ValidationHelper.GetInteger(siteSelector.Value, CMSContext.CurrentSiteID);
    }
    ...
    private void PreselectFilter(string filter)
    {
        ...
        if (board != "")
        {
            if (boardSelector.UniSelector.HasData)
            {
    ...
    public void ReloadFilter()
    {
        if (GroupID == 0)
        {
            plcSite.Visible = CMSContext.CurrentUser.IsGlobalAdministrator;
        }
        ...
        if (BoardID == 0)
        {
            // Init board selector
            boardSelector.IsLiveSite = IsLiveSite;
            boardSelector.SiteID = CMSContext.CurrentSiteID;
            boardSelector.GroupID = GroupID;
    ...
    protected object gridElem_OnExternalDataBound(object sender, string sourceName, object parameter)
    {
        switch (sourceName.ToLowerCSafe())
        {
            ...
            case "edit":
                ...
                editButton.OnClientClick = "modalDialog('" + CMSContext.ResolveDialogUrl(((EditPageUrl == "") ? url : EditPageUrl)) +
    ...
    protected void gridElem_OnAction(string actionName, object actionArgument)
    {
        ...
        RaiseOnAction(actionName, ((argument == null) ? actionArgument : argument));
    }
