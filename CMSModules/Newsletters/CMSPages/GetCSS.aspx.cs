...
public partial class CMSModules_Newsletters_CMSPages_GetCSS : GetFilePage
{
    ...
    private string newsletterTemplateName = null;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        using (CachedSection<CMSOutputResource> cs = new CachedSection<CMSOutputResource>(ref outputFile, CacheMinutes, true, cacheKey))
        {
            if (cs.LoadData)
            {
                // Process the file
                ...
                if ((outputFile != null) && cs.Cached)
                {
                    ...
                    CacheDependency cd = CacheHelper.GetCacheDependency(new string[] { "newsletter.emailtemplate|byname|" + newsletterTemplateName.ToLowerCSafe() });
    ...
    protected void ProcessStylesheet()
    {
        ...
        {
            // Get the template
            et = EmailTemplateInfoProvider.GetEmailTemplateInfo(newsletterTemplateName, CMSContext.CurrentSiteID);
            if (et != null)
            {
