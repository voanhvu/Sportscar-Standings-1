...
public partial class CMSModules_Newsletters_Controls_EditIssue : CMSAdminControl
{
    ...
    private bool mIsDialogMode = true;
    ...
    protected void lnkToggleAdvanced_Click(object sender, EventArgs e)
    {
        ...
        ScriptManager.RegisterStartupScript(this.Page, typeof(string), "ResizeContent_" + this.ClientID, "if (SetIFrameHeight) { SetIFrameHeight(); }", true);
    }
    ...
    public void RefreshEditableRegions()
    {
        ...
        ScriptHelper.RegisterStartupScript(this.Page, typeof(string), "SetRegionContent", string.Format("SetRegionContent({0});", ScriptHelper.GetString(hdnIssueContent.Value)), true);
    }
...
function SetRegionContent(regcontent) {{
    ...
    if ((regcontent != null) && (F != null)) {{
        F.onload = function() {{
            if (F.contentWindow.SetRegionContent != null) {{
                F.contentWindow.SetRegionContent(regcontent);
            }}
        }}
    }}
    return false;
