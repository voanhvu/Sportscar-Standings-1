...
public partial class CMSModules_Newsletters_Controls_SubscriberBlockedFilter : CMSAbstractBaseFilterControl
{
    ...
    {
        mBounceLimit = NewsletterHelper.BouncedEmailsLimit(CMSContext.CurrentSiteName);
    }
        ...
        switch (ddlBounceFilter.SelectedValue)
        {
            ...
            case Active:
                return string.Format("(((SubscriberType IS NULL OR SubscriberType='{0}' OR SubscriberType='{1}') AND (SubscriberBounces IS NULL OR SubscriberBounces<{2})) OR (SubscriberType = '{3}') OR (SubscriberType = '{4}'))",
                                     SiteObjectType.USER, PredefinedObjectType.CONTACT, mBounceLimit, SiteObjectType.ROLE, PredefinedObjectType.CONTACTGROUP);

            case Blocked:
                return string.Format("((SubscriberType IS NULL OR SubscriberType='{0}' OR SubscriberType='{1}') AND (SubscriberBounces>={2}))", SiteObjectType.USER, PredefinedObjectType.CONTACT, mBounceLimit);
