...
public partial class CMSModules_Newsletters_FormControls_NewsletterSubscriberSelector : FormEngineUserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (ShowSiteFilter)
        {
            ...
            usSubscribers.SetValue("DefaultFilterValue", (SiteID > 0) ? SiteID : CMSContext.CurrentSiteID);
        ...
        if (!ShowSiteFilter && (SiteID >= 0))
        {
            ...
            usSubscribers.WhereCondition = SqlHelperClass.AddWhereCondition(usSubscribers.WhereCondition, where);
        }

        // Limit selection only to subscribers, remove roles and users
        usSubscribers.WhereCondition = SqlHelperClass.AddWhereCondition(usSubscribers.WhereCondition, "(SubscriberType IS NULL)");
    }
