...
public partial class CMSModules_OnlineMarketing_Dialogs_ContentPersonalizationVariantEdit : CMSVariantDialogPage
{
    ...
    protected void UIFormControl_OnBeforeSaved(object sender, EventArgs e)
    {
        if (editElem.ValidateData())
        {
            ...
            parameters.Add("displayname", ValidationHelper.GetString(editElem.UIFormControl.FieldControls["VariantDisplayName"].Value, string.Empty));
            parameters.Add("codename", editElem.VariantCodeName);
            parameters.Add("description", ValidationHelper.GetString(editElem.UIFormControl.FieldControls["VariantDescription"].Value, string.Empty));
            parameters.Add("enabled", ValidationHelper.GetBoolean(editElem.UIFormControl.FieldControls["VariantEnabled"].Value, false));
            parameters.Add("condition", ValidationHelper.GetString(editElem.UIFormControl.FieldControls["VariantDisplayCondition"].Value, string.Empty));
