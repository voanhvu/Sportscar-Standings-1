...
public partial class CMSModules_Polls_Controls_View_PollView : CMSUserControl
{
    ...
    public DataSet Answers
    {
        get
        {
            if (DataHelper.DataSourceIsEmpty(answers))
            {
                ...
                using (CachedSection<DataSet> cs = new CachedSection<DataSet>(ref answers, CacheMinutes, true, null, "pollanswers", PollCodeName))
                {
                    if (cs.LoadData)
                    {
                        // Get from database
                        if (pi != null)
                        {
                            answers = PollAnswerInfoProvider.GetAnswers(pi.PollID, -1, "AnswerID, AnswerText, AnswerCount, AnswerEnabled, AnswerForm, AnswerAlternativeForm, AnswerHideForm");
                        }
    ...
    protected bool HasPermission()
    {
        ...
        if (pi != null)
        {
            ...
            if ((pi.PollAccess == SecurityAccessEnum.GroupMembers) && (CMSContext.CurrentUser.IsGroupMember(PollGroupID)))
            {
                toReturn = true;
            }
    ...
    private void Form_AfterDataLoad(object sender, EventArgs e)
    {
        BizForm bizForm = sender as BizForm;
        if ((bizForm != null) && (bizForm.BasicForm != null))
        {
            bizForm.BasicForm.SubmitButton.Visible = false;
            bizForm.BasicForm.SubmitImageButton.Visible = false;
        }
    }
