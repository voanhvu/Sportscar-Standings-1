...
public partial class CMSModules_PortalEngine_Controls_Editable_EditableImage : CMSUserControl
{
    ...
    public bool DisplaySelectorTextBox
    {
        get
        {
            return ValidationHelper.GetBoolean(GetValue("DisplaySelectorTextBox"), selPath.ImagePathTextBox.Visible);
        }
        set
        {
    ...
    public string GetContent()
    {
        if (!StopProcessing)
        {
            ...
            switch (ViewMode)
            {
                case ViewModeEnum.Edit:
                case ViewModeEnum.EditDisabled:
                    string path = URLHelper.UnResolveUrl(selPath.Value.Trim(), URLHelper.ApplicationPath);
                    if (!string.IsNullOrEmpty(path))
                    {
                        XmlDocument xml = new XmlDocument();
