...
public partial class CMSModules_PortalEngine_Controls_Editable_EditableText : CMSUserControl
{
    ...
    private void Page_PreRender(object sender, EventArgs e)
    {
        if (!StopProcessing)
        {
            ViewModeEnum viewMode = (CMSContext.ViewMode == ViewModeEnum.Preview) ? ViewModeEnum.Preview : ViewMode;
        ...
        if (!StopProcessing)
        {
            ...
            switch (ViewMode)
            {
                ...
                case ViewModeEnum.EditDisabled:
                    switch (RegionType)
                    {
                        case CMSEditableRegionTypeEnum.HtmlEditor:
                            // HTML editor
                            if ((forceReload || (!RequestHelper.IsPostBack()) || (ViewMode != ViewModeEnum.Edit)) && (htmlValue != null))
                            {
                ...
                default:
                    ...
                    // Only published
                    if ((CMSContext.ViewMode != ViewModeEnum.LiveSite) || !SelectOnlyPublished || ((CurrentPageInfo != null) && CurrentPageInfo.IsPublished))
                    {
                        if (isAuthorized)
                        {
                            if (ltlContent == null)
                            ...
                            if (ltlContent != null)
                            {
                                ltlContent.Text = CMSContext.CurrentResolver.ResolveMacros(content);
    ...
    public bool UsesHtmlEditor(string toolbarLocation)
    {
        if ((RegionType == CMSEditableRegionTypeEnum.HtmlEditor) && (ViewMode == ViewModeEnum.Edit))
        {
            return (toolbarLocation == null) || (toolbarLocation == HtmlAreaToolbarLocation);
        }
        return false;
    }
