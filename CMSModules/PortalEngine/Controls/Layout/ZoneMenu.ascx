            ...
            <ItemTemplate>
                <cms:ContextMenuContainer runat="server" ID="cmcZoneVariants" MenuID="moveToZoneVariants"
                    ...
                    <cms:ContextMenuItem runat="server" ID="zoneElem" onmouseover='<%#(((CMSWebPartZone)Container.DataItem).HasVariants) ? "SetContextMenuParameter(\"selectedMoveToZoneId\", \"" + ((CMSWebPartZone)Container.DataItem).ID + "\");" : "CM_Close(\"moveToZoneVariants\");"%>'
                        onclick='<%#"CM_Close(\"webPartZoneMenu\"); ContextMoveWebPartsToZone(GetContextMenuParameter(\"webPartZoneMenu\"), \"" + ((CMSWebPartZone)Container.DataItem).ID + "\");"%>' Text='<%#(((CMSWebPartZone)Container.DataItem).ZoneTitle != "" ? HTMLHelper.HTMLEncode(((CMSWebPartZone)Container.DataItem).ZoneTitle) : ((CMSWebPartZone)Container.DataItem).ID) + (((CMSWebPartZone)Container.DataItem).HasVariants ? "..." : "")%>' ImageUrl='<%# GetImageUrl("CMSModules/CMS_PortalEngine/ContextMenu/Variants/zone.png") %>' />
...
<script type="text/javascript">
    ...
    function ContextConfigureWebPartZone(definition) {
        ConfigureWebPartZone(escape(definition[0]), escape(definition[1]), escape(definition[2]), escape(definition[3]));
    }
