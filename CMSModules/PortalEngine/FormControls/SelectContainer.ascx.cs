...
public partial class CMSModules_PortalEngine_FormControls_SelectContainer : FormEngineUserControl
{
    ...
    private void SetDialog(bool allowEdit, bool allowNew)
    {
        ...
        string newUrl = "~/CMSModules/PortalEngine/UI/WebContainers/Container_New.aspx?editonlycode=true";

        if (aliasPath != String.Empty)
        {
            editUrl = URLHelper.AddParameterToUrl(editUrl, "aliaspath", aliasPath);
            newUrl = URLHelper.AddParameterToUrl(newUrl, "aliaspath", aliasPath);
        }

        if (instanceGUID != Guid.Empty)
        {
            editUrl = URLHelper.AddParameterToUrl(editUrl, "instanceGUID", instanceGUID.ToString());
            newUrl = URLHelper.AddParameterToUrl(newUrl, "instanceGUID", instanceGUID.ToString());
        }
        ...
        if (allowNew)
        {
            ...
            selectContainer.NewItemPageUrl = newUrl;
        }
    }
