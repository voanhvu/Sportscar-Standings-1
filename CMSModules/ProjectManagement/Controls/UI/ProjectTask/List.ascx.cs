...
public partial class CMSModules_ProjectManagement_Controls_UI_ProjectTask_List : CMSAdminListControl, IPostBackEventHandler, INamingContainer
{
    #region "Variables"
    ...
    private string mProjects = String.Empty;
    ...
    private void ShowReminderPopup()
    {
        ...
        titleElem.TitleImage = GetImageUrl("Objects/PM_ProjectTask/reminder.png");
    }
    ...
    private void BuildConditions()
    {
        ...
        if (!IgnoreCommunityGroup)
        {
            ...
            {
                gridElem.WhereCondition = SqlHelperClass.AddWhereCondition(gridElem.WhereCondition, "ProjectGroupID = " + CommunityGroupID);
            }
            else
            {
                gridElem.WhereCondition = SqlHelperClass.AddWhereCondition(gridElem.WhereCondition, "ProjectGroupID IS NULL");
            }
        }
        ...
        if (ProjectID > 0)
        {
            gridElem.WhereCondition = SqlHelperClass.AddWhereCondition(gridElem.WhereCondition, "(ProjectSiteID IS NULL) OR (ProjectSiteID = " + CMSContext.CurrentSiteID) + ")";
        }
        ...
        else
        {
            ...
            if (!String.IsNullOrEmpty(SiteName))
            {
                ...
                if (si != null)
                {
                    gridElem.WhereCondition = SqlHelperClass.AddWhereCondition(gridElem.WhereCondition, "(ProjectSiteID IS NULL) OR  (ProjectSiteID = " + si.SiteID + ")");
                }
                // If sitename wasn't found do not display any records
                else
                {
                    gridElem.WhereCondition = SqlHelperClass.AddWhereCondition(gridElem.WhereCondition, "1=2");
                }
            }
        }
        ...
        if (ProjectID > 0)
        {
            ...
            gridElem.WhereCondition = SqlHelperClass.AddWhereCondition(gridElem.WhereCondition, "ProjectTaskProjectID =" + ProjectID);
        }
    ...
    private bool GetSortingArrowsVisibility()
    {
        ...
        if (ProjectID > 0)
        {
            if (ProjectInfoProvider.IsAuthorizedPerProject(ProjectID, ProjectManagementPermissionType.MODIFY, CMSContext.CurrentUser)
                || CMSContext.CurrentUser.IsAuthorizedPerResource("CMS.ProjectManagement", PERMISSION_MANAGE))
            {
                arrowsVisible = true;
