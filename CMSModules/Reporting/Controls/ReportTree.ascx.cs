...
public partial class CMSModules_Reporting_Controls_ReportTree : CMSAdminControl
{
    ...
    public int MaxTreeNodes
    {
        get
        {
            if (mMaxTreeNodes < 0)
            {
                mMaxTreeNodes = SettingsKeyProvider.GetIntValue(CMSContext.CurrentSiteName + ".CMSMaxUITreeNodes");
            }
    ...
    private TreeNode treeElem_OnNodeCreated(DataRow itemData, TreeNode defaultNode)
    {
        if (UseMaxNodeLimit)
        {
            ...
            if (mIndex == MaxTreeNodes)
            {
                ...
                node.Text = "<span class=\"ContentTreeItem\" onclick=\"SelectReportNode(" + parentID + " ,'reportcategory'," + parentParentID + ",true ); return false;\"><span class=\"Name\" style=\"font-style: italic;\">" + GetString("general.seelisting") + "</span></span>";
                return node;
            }
            if (mIndex > MaxTreeNodes)
            {
