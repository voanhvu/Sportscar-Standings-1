...
public partial class CMSModules_Scoring_Controls_UI_Rule_Edit : CMSAdminEditControl
{
    ...
    protected override void OnPreRender(EventArgs e)
    {
        ...
        {
            foreach (DictionaryEntry kvp in activityFormCondition.FieldLabels)
            {
                ((LocalizedLabel)kvp.Value).CssClass = "EditingFormLabel";
            }
        }
    }
    ...
    private void ucActivityType_OnSelectedIndexChanged(object sender, EventArgs e)
    {
    ...
    {
        AddHeaderAction(new SaveAction(Page));
        ...
        if (rule != null)
        {
            AddHeaderAction(new HeaderAction
            {
                ...
                ImageUrl = GetImageUrl("Objects/OM_Score/rule_add.png"),
                Text = GetString("om.score.newrule"),
                RedirectUrl = ResolveUrl("Tab_Rules_Edit.aspx?scoreid=" + scoreId)
            });
        }
    ...
    protected bool ValidateForm()
    {
        ...
        {
            DisplayError(formCondition.ValidationErrorMessage);
            return false;
        }
    ...
    {
        ShowError(TextHelper.LimitLength(errorMessage, 200), errorMessage, null);
    }
