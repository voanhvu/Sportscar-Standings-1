...
public partial class CMSModules_Settings_Controls_SettingsGroupViewer : CMSAdminEditControl, ICallbackEventHandler
{
                ...
                else
                {
                    ...
                    if (control != null)
                    {
                        ...
                        else
                        {
                            ...
                            lblError.Text = String.IsNullOrEmpty(control.ErrorMessage) ? GetString("Settings.ValidationError") : control.ErrorMessage;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
                    ...
                    if (element != null) 
                    {
                        ...
                        if (element.nodeName.toLowerCase() == 'table') 
                        {
                            ...
                            if ((children != null) && (children.length > 0)) 
                            {
                                ...
                                type = children[0].type.toLowerCase() + 'list';
                            }
                        }
                        else 
                        {
                    ...
                    if(element != null) 
                    {
                        ...
                        if ((children != null) && (children.length > 0)) 
                        {
                            for (var i = 0; i < children.length; i++) 
                            {
                                ...
                                try
                                {
                                   children[i].parentNode.disabled = disabled;
                                }
                                catch(e) {}
                            }
                        }
        ...
        foreach (SettingsCategoryInfo group in GetGroups())
        {
            ...
            if (keys != null)
            {
                ...
                foreach (SettingsKeyInfo key in keys)
                {
                    ...
                    if (control != null)
                    {
                        string defValue = SettingsKeyProvider.GetValue(key.KeyName.ToLowerCSafe());
                        string value = null;
                        ...
                        else
                        {
                            ...
                            value = SettingsKeyProvider.GetValue(GetFullKeyName(key.KeyName).ToLowerCSafe());
                        }
                        control.Value = value;
                    ...
                    {
                        switch (key.KeyType.ToLowerCSafe())
                        {
                            case "boolean":
                    ...
                    AddLiteral(p, @"<td class=""EditingFormRightBorder"">&nbsp;</td></tr>");
        ...
        if (groupNo > 0)
        {
            ...
            if (!URLHelper.IsPostback() && QueryHelper.GetInteger("resettodefault", 0) == 1)
            {
    ...
    private IEnumerable<SettingsKeyInfo> GetSearchedKeys(IEnumerable<SettingsKeyInfo> keys)
    {
        foreach (SettingsKeyInfo key in keys)
        {
            ...
            if ((keyDisplayName.ToLowerCSafe().Contains(mSearchText.ToLowerCSafe())) || ((mSearchDescription) && (keyDescription.ToLowerCSafe().Contains(mSearchText.ToLowerCSafe()))))
            {
                yield return key;
            }
        }
    ...
    {
        return mKeyItems.Find(x => x.ValueElementClientID.EqualsCSafe(clientID, true));
    }
    ...
    /// </summary>
    /// <param name="p">CategoryPanel instance</param>
    /// <param name="text">Text representing LiteralControl</param>
    ...
    /// </summary>
    /// <param name="p">CategoryPanel whose control collection should be modified</param>
    /// <param name="ctl">Control that should be added to the <c>CategoryPanel controls collection</param>
    /// <param name="literalBefore">If not null new LiteralControl will be added before the control</param>
    /// <param name="literalAfter">If not null new LiteralControl will be added after the control</param>
    ...
    private CategoryPanel GetCategoryPanel(SettingsCategoryInfo group, int groupNo)
    {
        ...
        if ((!string.IsNullOrEmpty(mSearchText)) && (mSearchText.Length >= mSearchLimit))
        {
            title = GetCategoryNames(group.CategoryIDPath);
        }
        else
        {
            title = group.CategoryDisplayName;
        }

        // Resolve title
        p.Text = HTMLHelper.HTMLEncode(ResHelper.LocalizeString(title));
    ...
    /// </summary>
    /// <param name="settingsKey">SettingsKeyInfo instance representing the current processing key</param>
    /// <param name="groupNo">Number representing index of the proccesing settings group</param>
    /// <param name="keyNo">Number representing index of the proccesing SettingsKeyInfo</param>
    /// <param name="inheritChecked">Output parameter indicating whether the inherit CheckBox should be checked</param>
    /// <param name="chkInherit">Inherit CheckBox instance</param>
    ...
    private CheckBox GetCheckBox(SettingsKeyInfo settingsKey, int groupNo, int keyNo, out bool inheritChecked, CheckBox chkInherit, out string inputControlID, out string value)
    {
        ...
        {
            inheritChecked = true;
            chbox.Checked = SettingsKeyProvider.GetBoolValue(settingsKey.KeyName.ToLowerCSafe());
        }
        else
        {
            inheritChecked = false;
            chbox.Checked = SettingsKeyProvider.GetBoolValue(GetFullKeyName(settingsKey.KeyName).ToLowerCSafe());
        }
    ...
    /// <summary>
    /// Gets <c>Image</c> instance for the input SettingsKeyInfo object.
    /// </summary>
    /// <param name="settingsKey"><c>SettingsKeyInfo</c> instance</param>
    /// <param name="groupNo">Number representing index of the proccesing settings group</param>
    /// <param name="keyNo">Number representing index of the proccesing SettingsKeyInfo</param>
    ...
    public void RaiseCallbackEvent(string eventArgument)
    {
        ...
        mCallBackResult = string.Format(@"{0}|{1}|{2}|{3}|{4}|{5}",
                                        clientId, value, ski.UpdatePanelClientID, clientId.EqualsCSafe(UNDEFINED_VALUE_ELEMENT_CLIENT_ID, true), "", "");
    }
