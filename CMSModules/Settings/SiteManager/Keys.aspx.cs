...
";
    ...
    private string GetImageUrlForHeader(string categoryName)
    {
        ...
        if (!string.IsNullOrEmpty(categoryName))
        {
            ...
            {
                imageUrl = UIHelper.ResolveImageUrl(imageUrl);
            }
            // Otherwise use default icon
            else
            {
                imageUrl = GetImageUrl("CMSModules/CMS_Settings/Categories/module.png");
            }
        }
    ...
    private string GetHelpTopicName()
    {
        ...
        {
            string lowCatName = SettingsGroupViewer.SettingsCategoryInfo.CategoryName.ToLowerCSafe();

            if (lowCatName.StartsWithCSafe("cms."))
            {
    ...
    private void SaveSettings()
    {
        ...
        if (SettingsGroupViewer.CategoryName.ToLowerCSafe() == "cms.system.debug")
        {
            CMSFunctions.ResetDebugSettings();
        }
        else if (SettingsGroupViewer.CategoryName.ToLowerCSafe() == "cms.system.performance")
        {
