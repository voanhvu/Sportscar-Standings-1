...
public partial class CMSModules_WebFarm_Controls_WebFarm_AnonymousTask_Filter : CMSAbstractBaseFilterControl
{
    #region "Properties"
    ...
    protected string GenerateWhereCondition()
    {
        ...
        {
            case "notprocessed":
                where = SqlHelperClass.AddWhereCondition(where, "TaskErrorMessage IS NULL");
                break;

            case "failed":
                where = SqlHelperClass.AddWhereCondition(where, "TaskErrorMessage IS NOT NULL");
                break;
        }
        return where;
    }
