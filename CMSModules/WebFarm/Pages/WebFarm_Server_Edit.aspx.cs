...
public partial class CMSModules_WebFarm_Pages_WebFarm_Server_Edit : SiteManagerPage
{
    ...
    {
        CurrentMaster.Title.HelpTopicName = "webfarm_newedit_server";
        ...
        if (serverid > 0)
        {
            ...
            if (wi == null)
            {
                ShowError(GetString("WebFarmServers_Edit.InvalidServerID"));
        ...
        if ((ValidationHelper.GetString(Request.QueryString["saved"], "") != "")  && !URLHelper.IsPostback())
        {
            ShowChangesSaved();
        }
    }
    ...
    protected void btnOK_Click(object sender, EventArgs e)
    {
        ...
        if (result == "")
        {
            ...
            {
                ShowError(ex.Message.Replace("%%name%%", wi.ServerName));
            }
        }
        else
        {
