...
var JsPlumbAbstractNode = $class({
    ...
    */
    printNode: function () {
        ...
        if (this.definition.HasTargetPoint) {
            this.targetPoint.initDropTarget($j("#" + this.htmlId));
        }
    ...
    */
    getDefaultHtmlElement: function () {
        var elementJQ = $j('<div class="Node ' + this.nodeStyle + '" id="' + this.parent.attr('id') + '_' + this.id + '">' +
    ...
    */
    getEditIconHtmlRepresentation: function (title) {
        if (this.readOnly)
            return '';
        return '<div class="button pen" title="' + title + '" alt="Edit"></div>';
    },
    ...
    */
    getDeleteIconHtmlRepresentation: function (title) {
        if (this.readOnly || !this.definition.IsDeletable)
            return '';
        return '<div class="button del" title="' + title + '" alt="Delete"></div>';
    },
    ...
    */
    getContentHtmlRepresentation: function () {
        var background = 'class="main"';
        if (this.thumbnailImageUrl)
            background = ' class="icon" style=\"background-image: url(\'' + this.thumbnailImageUrl + '\');\"';

        return '<div class="content gray gradient"><div ' + background + '>' + this.content + '</div>' + this.getFooterHtmlRepresentation() + '</div>';
    },
    ...
    */
    getCleanedString: function (string) {
        if (!string)
            return '';
        return $j("<span/>").text(string).html();
    },
    ...
    */
    getFooterHtmlRepresentation: function () {
        if (this.hasTimeout) {
            return '<div class="hr"></div><div class="time" title="' + this.graph.getResourceString("TimeoutIconTooltip") + '"></div>';
        }
        return '';
    },
    ...
    */
    ensureEndpointTooltip: function (endpoint) {
        var canvasJQ = $j(endpoint.canvas);
    ...
    */
    openEditModalDialog: function (sourcePointId) {
        var url = this.graph.addresses["EditNodeDialog"];
        if (url) {
            url += "?objectid=" + this.id + "&dialog=1&graph=" + this.graph.jsObjectName + this.getSourcepointQueryString(sourcePointId);
            modalDialog(url, "Edit", "950", "650");
        }
    },
