var JsPlumbConditionNode = $class({
    ...
    */
    addConditionRow: function (sourcePoint) {
        ...
        $j('#' + this.htmlId + ' div.Cases').append(this.getCaseRowTemplate(sourcePoint));
        $j('#' + sourcePoint.HtmlID).data('id', sourcePoint.ID);
    ...
    */
    createConditionRowTooltip: function (sourcePoint) {
        ...
        $j('#' + tooltipId).remove();
        var tooltip = $j('<div id="' + tooltipId + '" class="tooltip CustomTooltip">' + sourcePoint.Tooltip + '</div>');
        this.graph.graphJQ.parents("body").append(tooltip);
        var tooltipSettings = this.getConditionTooltipSettings(tooltipId);
        $j('#' + tooltipId).hide();
        $j("#" + sourcePoint.HtmlID).find(".Editable").tooltip(tooltipSettings);
    },
    ...
    */
    getConditionTooltipSettings: function (tooltipId) {
        ...
        if (isRTL) {
            tooltipSettings.position = "bottom right";
            tooltipSettings.offset = [2, -$j('#' + tooltipId).width()];
        } else {
            tooltipSettings.position = "bottom left";
            tooltipSettings.offset = [2, $j('#' + tooltipId).width()];
        }
    ...
    */
    getNodeIconHtmlRepresentation: function () {
        ...
        return '<div class="icon" alt="NodeIcon" ' + tooltip + '></div>';
    },
    ...
    */
    getAnchorRelativeTopOffset: function (switchCaseId) {
        var rowJQ = $j("#" + this.getHtmlRowId(switchCaseId));
    ...
    */
    removeCaseRow: function (caseId) {
        $j("#" + this.getHtmlRowId(caseId)).remove();
        $j("#" + this.getHtmlRowId(caseId) + "tooltip").remove();
