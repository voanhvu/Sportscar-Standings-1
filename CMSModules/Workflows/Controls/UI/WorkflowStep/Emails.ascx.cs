...
public partial class CMSModules_Workflows_Controls_UI_WorkflowStep_Emails : CMSUserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        ucApprove.TemplateType = EmailTemplateMacros.EmailType.WorkflowEmail;
        ucReadyApproval.TemplateType = EmailTemplateMacros.EmailType.WorkflowEmail;
        ucReject.TemplateType = EmailTemplateMacros.EmailType.WorkflowEmail;
        pnlSettings.GroupingText = GetString("general.general");
        pnlTemplates.GroupingText = GetString("workflow.templates");
        ...
        chkEmails.SetDefaultValue(Workflow.SendEmails(CMSContext.CurrentSiteName, WorkflowEmailTypeEnum.Unknown));
        chkEmails.AutoPostBack = true;
        chkEmails.CheckedChanged += SendNotification_Changed;

        chkApprove.SetDefaultValue(Workflow.SendEmails(CMSContext.CurrentSiteName, WorkflowEmailTypeEnum.Approved));
        chkApprove.AutoPostBack = true;
        chkApprove.CheckedChanged += SendApproveNotification_Changed;

        chkReadyApproval.SetDefaultValue(Workflow.SendEmails(CMSContext.CurrentSiteName, WorkflowEmailTypeEnum.ReadyForApproval));
        chkReadyApproval.AutoPostBack = true;
        chkReadyApproval.CheckedChanged += SendReadyApprovalNotification_Changed;

        chkReject.SetDefaultValue(Workflow.SendEmails(CMSContext.CurrentSiteName, WorkflowEmailTypeEnum.Rejected));
