...
public partial class CMSModules_Workflows_Controls_UI_WorkflowStep_SourcePoint_List : CMSAdminListControl
{
    ...
    private string mItemEditUrl = "~/CMSModules/Workflows/Pages/WorkflowStep/SourcePoint/frameset.aspx";
    ...
    private string GetDecoratedCondition(string label)
    {
        ...
        {
            return String.IsNullOrEmpty(label) ? String.Empty : String.Format("<b>{0}</b><div>{1}</div>", GetString("workflowstep.sourcepoint.incase"), label);
        }
        ...
        if (String.IsNullOrEmpty(label))
        {
            ...
            {
                return String.Format("<b>{0}</b>", GetString("workflowstep.sourcepoint.else"));
            }
            else
            {
                return String.Empty;
        ...
        else
        {
            ...
            {
                return String.Format("<b>{0}</b><div>{1}</div>", GetString("workflowstep.sourcepoint.if"), label);

            }
            else
            {
                return String.Format("<b>{0}</b><div>{1}</div>", GetString("workflowstep.sourcepoint.incase"), label);
            }
        }
