...
using CMS.UIControls.Workflows;
...
public partial class CMSModules_Workflows_Controls_WorkflowDesignerToolbar : UniGraphToolbar
{
    ...
    protected StepTypeUniMenuItems Factory
    {
        get
        {
            if (mInjector == null)
            {
                mInjector = new StepTypeUniMenuItems(JsGraphObject, Page);
            }
            return mInjector;
        }
    }
    ...
    private void AddActionGroup(string where)
    {
        ...
        // Allowed objects
        condition = SqlHelperClass.AddWhereCondition(condition, "ActionAllowedObjects IS NULL OR ActionAllowedObjects LIKE N'%" + Workflow.WorkflowAllowedObjects + "%'");
