...
public partial class CMSPages_GetTestingModeReport : CMSPage
{
    ...
    private string GetDebug()
    {
        ...
        for (int i = RequestHelper.LastLogs.Count - 1; i >= 0; i--)
        {
            ...
            if (log != null)
            {
                ...
                if (logs.Count > 1)
                {
                    DataTable table = AllLog.MergeTables(logs, Page, false);
    ...
    private string GetSystemInfo()
    {
        ...
        sb.AppendLine(GetRow("Administration-System.Account", WindowsIdentity.GetCurrent().Name));
        sb.AppendLine(GetRow("Administration-System.Version", Environment.Version.ToString()));
        ...
        sb.AppendLine(GetEmptyRow());
        sb.AppendLine(GetRow("Administration-System.IP", HTTPHelper.UserHostAddress));
        sb.AppendLine(GetRow("Administration-System.MachineName", HTTPHelper.MachineName));
        sb.AppendLine("</table>");
        ...
        sb.AppendLine(GetRow("Administration-System.ServerName", tm.DatabaseServerName));
        sb.AppendLine(GetRow("Administration-System.ServerVersion", tm.DatabaseServerVersion));
        sb.AppendLine(GetEmptyRow());
        sb.AppendLine(GetRow("Administration-System.DatabaseName", tm.DatabaseName));
        sb.AppendLine(GetRow("Administration-System.DatabaseSize", tm.DatabaseSize));
        sb.AppendLine("</table>");
