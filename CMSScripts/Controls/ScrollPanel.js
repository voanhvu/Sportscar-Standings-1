...
function scrollPanelInit(scrollContainer, forceReload) {
    ...
    if (scrollPanel.Container === null) {
        scrollPanel.Container = jQuery('#' + scrollContainer);
    }
    ...
    if (scrollPanel.ScrollAreaContainer === null) {
        scrollPanel.ScrollAreaContainer = jQuery('#' + scrollPanel.InnerContainerId + ':first', scrollPanel.Container);
    }

    if (scrollPanel.SeparatorItem === null) {
        if (scrollPanel.ItemSeparatorClass.length > 0) {
            scrollPanel.SeparatorItem = jQuery('.' + scrollPanel.ItemSeparatorClass + ':first', scrollPanel.ScrollAreaContainer);
        }
    }
    ...
    if (scrollPanel.LastItem === null) {
        ...
        if (scrollPanel.ItemClass.length > 0) {
            var items = jQuery('.' + scrollPanel.ItemClass, scrollPanel.Container);
...
function spSetupForwardScroll(scrollContainer, forceReload) {
    ...
    if (lastItem.length > 0) {
        if (!isVerticalLayout) {
            ...
            var lastItemMargin = parseInt(lastItem.css('margin-top'), 10);
