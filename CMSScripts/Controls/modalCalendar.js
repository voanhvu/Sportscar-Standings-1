function localizeRangeCalendar(mainDiv, input, isRTL, isFixed) {
    ...
    var viewWidth = document.documentElement.clientWidth + $j(document).scrollLeft();
    var viewHeight = document.documentElement.clientHeight + $j(document).scrollTop();
    var offset = input.offset();

    offset.top += inputHeight + 1;

    offset.left -= isRTL ? (dpWidth - inputWidth) : 0;
    offset.left -= (isFixed && offset.left == input.offset().left) ? $j(document).scrollLeft() : 0;
    offset.top -= (isFixed && offset.top == (input.offset().top + inputHeight)) ? $j(document).scrollTop() : 0;
