...
jQuery(document).ready(function () {
    dialogZIndex = 30000;
    OEHighlightContainer = jQuery('<div id="OEHighlightContainer"></div>');
    jQuery(document.body).append(OEHighlightContainer);

    jQuery('.OnSiteWebPartBegin').each(function () {
    ...
    OEHighlightContainer
        .on("mouseover", ".OnSiteHighlight", function () {
            if (OEActiveWebPart == null) {
                var jContainer = jQuery(this).parent();
    ...
    jQuery('.OnSiteContextMenu')
        .bind('mouseenter', function (e) {
            clearTimeout(OEContextMenuTimer);
            jQuery(this).show();
            OEActivateMenuButton(jQuery(this).data("buttonEl"));
        })
    ...
    jQuery('.UIToolbar .BigButton').mouseleave(function () { OEDeactivateMenuButton(this); jQuery('.OnSiteContextMenu').hide(); });
});

// Proceed the web part to ensure highlighting or marking the web part as an "empty" web part
function ProceedItem(startSpanEl, ensureImageLateLoad) {
    var startSpan = jQuery(startSpanEl);
...
function OEHighlightMenuFitIntoScreen(highlightContainer) {
    var menuDiv = jQuery('.OnSiteMenuTable', highlightContainer);
    ...
    var cmsHeaderHeight = jQuery('#CMSHeaderDiv').height();
...
function OECalculateWebPartBorders(startSpan, webPartObj) {
    ...
    while (sibling != null) {
        ...
        updatedWebPartObj = OEUpdateDimension(jQuery(sibling), updatedWebPartObj, true);
        sibling = OENextElementSibling(sibling);
    }
...
function OEActivateWebPartBorder(webPartObj, event, refreshPosition, highlightMode) {
    ...
    var highlightContainer = jQuery('#OE_' + webPartObj.webPartSpanId, OEHighlightContainer);

    if (OEHighlightButtonStatus == OEHighlightStatusEnum.NONE) {
        // Hide all highlighting
        jQuery('.OnSiteHighlightContainer', OEHighlightContainer).hide();
    }

    if ((highlightContainer.get(0) == null) || refreshPosition) {
        // Render the web part border
        var startSpan = jQuery('#' + webPartObj.webPartSpanId);
...
function OEGetHighlightWebPartHtml(webPartObj) {
    var divBorder = jQuery('<div class="OnSiteHighlight"></div>');
    var cmsHeaderHeight = jQuery('#CMSHeaderDiv').height();
    ...
    if (configHtml.length > 0) {
        ...
        // Border container
        webPartBorderContainer = jQuery('<div id="OE_' + webPartObj.webPartSpanId + '" class="OnSiteHighlightContainer ' + (!OEIsRTL ? 'LTR' : '') + '" style="left: ' + webPartObj.startX + 'px; top: ' + webPartObj.startY + 'px;">' + borderTop + borderRight + borderBottom + borderLeft + '</div>');
        webPartBorderContainer.data('webPart', webPartObj);

        jQuery(OEHighlightContainer).append(webPartBorderContainer);
    }
...
function OEUpdateDimension(jObj, webPartObj, isFirstChildNode) {
    ...
    if (jObj.is(':visible') && !jObj.is('option') && (!jObj.is('br') || isFirstChildNode)) {
        ...
        jObj.children().each(function () {
            webPartObj = OEUpdateDimension(jQuery(this), webPartObj, false);
        });
    }
...
function OENextElementSibling(el) {
    ...
    while ((el != null) 
            && (
                ...
                || ((el.nodeType === 3) && (jQuery.trim(el.nodeValue).length === 0))
                )
            );
    ...
    if ((el != null) && (el.nodeType === 3)) {
        ...
        jQuery(el).wrap('<span class="OnSitePlainText"></span>');
        el = el.parentNode;
    }
...
function OEHighlightToggle(event, buttonEl) {
    ...
    jQuery('.OnSiteMenuTable', OEHighlightContainer).each(function () {
        OEHighlightRestoreMenuPosition(jQuery(this));
    });
    ...
    if (OEHighlightButtonStatus == OEHighlightStatusEnum.NONE) {
        ...
        jQuery('.WebPartBorder').mouseover();
    }
    else {
        ...
        jQuery('.WebPartBorderActive').mouseout();
    }
    ...
    if (scrollBarVisibleBeforeHighlight != scrollBarVisibleAfterHighlight) {
        ...
        if (scrollBarVisibleAfterHighlight) {
            ...
            tempDiv = jQuery('<div style="position: absolute;  top: ' + (jQuery(document).height() + 10) + 'px; width: 1px; height: 1px;"></div>');
...
function OEGenerateEmptyWebPartMenu() {
    ...
    OEEmptyWebPartMenu = jQuery('.OnSiteHiddenButton .UniMenuContextMenu');
    OEEmptyWebPartMenu.empty();

    var webPartOpeningDiv = '<div class="Item">';
    ...
    if (len > 0) {
        jQuery('.OnSiteHiddenButton').show();
    }
    else {
        jQuery('.OnSiteHiddenButton').hide();
    }
...
function OEEnsureImageLateLoad(element, startSpanEl) {
    ...
    if (element.tagName == "IMG") {
        ...
        jElement
            ...
            .load(function () {
                var startSpanEl = jQuery(this).data("startSpanEl");
                var index = OEEmptyWebPartSpanTags.indexOf(startSpanEl);
                if (index != -1) {
        ...
        jQuery('img', jElement).each(function () {
...
function OEGetWebPartConfigHtml(webPartSpanId, revertOrder) {
    ...
    if (dialogWidth.length == 0) {
        var webPartObj = jQuery(spanObj).next();
    ...
    if ((propertiesPageUrl.length > 0) || (editPageUrl.length > 0)) {
        ...
        var closeParentContextMenuScript = "jQuery(this).parents('.OnSiteContextMenu').hide(); ";
        ...
        var editHtml = '<div class="ActionIcon"><img src="' + OEEditImagePath + '" alt="edit" ' + onclick + '="' + editableScript + ';"></div>';
        var propertiesHtml = '<div class="ActionIcon"><img src="' + OEConfigureImagePath + '" alt="properties" ' + onclick + '="' + propertiesScript + ';" ></div>';

        configHtml = '<div class="Name">' + title + '</div>';
...
function OEActivateMenuButton(buttonEl, usePermanentCss) {
    ...
    jQuery(buttonEl).addClass(highlightCSSClass);
}
...
function OEDeactivateMenuButton(buttonEl, usePermanentCss) {
    ...
    jQuery(buttonEl).removeClass(highlightCSSClass);
}
...
function OEDeactivateMenuButtons() {
    jQuery(".UniMenuContent .Selected").each(function () {
...
function OEShowContextMenu(buttonEl, contextMenuName) {
    ...
    clearTimeout(OEContextMenuTimer);
    jQuery('.OnSiteContextMenu').hide();

    OEDeactivateMenuButtons();

    var jContextMenu = jQuery('.OnSiteContextMenu[data-contextmenu="' + contextMenuName + '"]');
    jButton = jQuery(buttonEl);
    jContextMenu.data("buttonEl", buttonEl);
    btnOffset = jButton.offset();
    jContextMenu.css('top', btnOffset.top - jQuery(window).scrollTop() + jButton.outerHeight());
...
function RefreshTree(nodeId, parentNodeId) {
    refreshPageOnClose = true;

    if ((nodeId > 0) && (nodeId != OECurrentNodeId) && (parentNodeId != OECurrentNodeId)) {
        // Show loading
        ToggleUpdateProgress(true);

        // Redirect to the specified document
        var reloadUrl = jQuery.param.querystring(window.location.href, "onsitenodeid=" + nodeId);
