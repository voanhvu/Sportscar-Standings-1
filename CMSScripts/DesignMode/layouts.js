...
function ChangeLayoutItem(clientId, offset, instanceGuid) {
    var id = clientId + "_current";
    curElem = Get(id);
    items = parseInt(Get(clientId + "_items").value);

    cur = parseInt(curElem.value);
    if ((cur >= 1) && (cur <= items)) {
    ...
    if ((cur >= 1) && (cur <= items)) {
        Get(clientId + "_item" + cur).style.display = 'block';
    }
    curElem.value = cur;
...
function PreviousLayoutItem(clientId) {
    ChangeLayoutItem(clientId, -1);
}

function NextLayoutItem(clientId) {
    ChangeLayoutItem(clientId, 1);
}
...
function AutoResize(clientId, cname, adjust) {
    ...
    try {
        ...
        if ((newwidth > parw - marg) && (par.parentNode.style.overflow == '')) {
            //newwidth = parw - marg;
        }
        if (s.width != (newwidth + 'px')) {
            if (par.originalWidth == null) {
                par.originalWidth = par.offsetWidth;
            }
            if (newwidth < par.originalWidth) {
                if (par.position == 'absolute') newwidth = par.originalWidth;
                else newwidth = par.parentNode.offsetWidth;
            }
            s.width = newwidth + 'px';
        }
        if (s.height != (newheight + 'px')) {
            if (par.originalHeight == null) par.originalHeight = par.offsetHeight;
            if (newheight < par.originalHeight) newheight = par.originalHeight;
...
function EditImageProperty(ev, e, wpid, propName, selUrl) {
    ...
    if (selUrl != null) {
        ...
        modalDialog(selUrl, "SelectImage", 900, 600);
    }

    ed.wpid = wpid;
    ed.propName = propName;

    var ev = jQuery.event.fix(ev);
    ev.stopPropagation();
    ev.preventDefault();
}
...
function CheckListClick(e, wpid, propName) {
    ...
    jQuery('.' + wpid + '_check').each(function (i, item) {
        if (item.checked) {
            res.push(item.value);
        }
    });
...
function EnsureColorEditor() {
    ...
    if (colorEdit == null) {
        colorEdit = jQuery('<div class="ColorEditor"></div>');
        ...
        jQuery.each(colors, function (i, col) {
            if (col == '') {
                colorEdit.append('<div class="ClearBoth"></div>');
                return;
            }
            var c = jQuery('<div class="ColorEditorColor" style="background-color: ' + col + ';">&nbsp;</div>');
            c.click(function () {
                var ed = colorEdit;
...
function EnsureEditControls(type) {
    ...
    if (propOverlay == null) {
        propOverlay = jQuery('<div class="EditOverlay">&nbsp</div>');
    ...
    switch (type.toLowerCase()) {
        ...
        case "textbox":
            if (textBoxEdit == null) {
                textBoxEdit = jQuery('<input class="EditProperty TextBoxField" type="text" />');
        ...
        default:
            if (textAreaEdit == null) {
                textAreaEdit = jQuery('<textarea wrap="off" class="EditProperty TextAreaMedium"></textarea>');
