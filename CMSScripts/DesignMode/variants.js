...
window.onbeforeunload = function () {
    if (typeof (existingOnBeforeUnload) == "function") { existingOnBeforeUnload(); }
...
function ToggleActionButtons(variantSliderId, enabled) {
    ...
    var inputElems = sliderObj.getElementsByTagName("input");
    for (var i = 0; i < inputElems.length; i++) {
        if (inputElems[i].className == 'SliderBtnEnabled') {
            inputElems[i].style.display = (enabled) ? 'block' : 'none';
        } else if (inputElems[i].className == 'SliderBtnDisabled') {
            inputElems[i].style.display = (!enabled) ? 'block' : 'none';
        }
    }
}
...
function AutoPostitionContextMenu(menuId) {
    ...
    if (menuEl != null) {
        ...
        var windowWidth = $j(window).width() + $j(window).scrollLeft();
        var windowHeight = $j(window).height() + $j(window).scrollTop();
...
function OnSetContextMenuParameter(menuId, params) {
    if ((menuId == "webPartMenu") || (menuId == "webPartZoneMenu")) {
        var zoneId = params[0];
        var webPartId = params[1];
        var instanceGuid = params[3];
        var displayMVTWebPartMenu = true;
        var displayMVTZoneMenu = true;
        var displayCPWebPartMenu = true;
        var displayCPZoneMenu = true;
        var isWebPartMenu = (params.length > 4);
        var showVariantsDenied = false;
...
// Updates the "webPartLocation" array and the variants array when dropping a webpart between zones 
function OnDropWebPartVariants(container, targetZoneId, webPartId, aliasPath, instanceGuid, templateId) {
    webPartLocation[container] = new Array(targetZoneId, webPartId, aliasPath, instanceGuid, templateId);
    var itemInfo = itemCodesAssociativeArray['Variant_WP_' + instanceGuid.replace(/-/g, '')];
    if (itemInfo != null) {
