...
function addHandle(o, win) {
    ...
    /*** CMS ***/
    //o.style.cursor = "move";
    $j(o).mouseenter(function () {
        if (window.visiblePopup) {
            if (!window.visiblePopup.data('fullScreen')) {
...
/*** CMS ***/
function DIF_cleanCollections() {
    var keys = Object.keys(DIF_handles);
    if (keys.length > 0) {
        for (var i = 0; i < keys.length; i++) {
            var name = keys[i];
            var x = 0;
            for (x = 0; x < DIF_handles[name].length; x++) {
                try {
                    // Check dead elements
                    var und = DIF_handles[name][x] == undefined;
                } catch (e) {
                    // Remove dead elements
                    DIF_handles[name].splice(x, 1);
                    delete DIF_iframeWindows[name];
                    delete DIF_iframeObjects[name];
                    continue;
                }
            ...
            if (DIF_handles[name].length == 0) {
                delete DIF_handles[name];
            }
        }
        if (DIF_handles == null) {
            DIF_handles = new Object();
        }
        if (DIF_iframeWindows == null) {
            DIF_iframeWindows = new Object();
        }
        if (DIF_iframeObjects == null) {
            DIF_iframeObjects = new Object();
        }
    }
}
/*** END CMS ***/
...
function DIF_getObjectXY(o) {
    ...
    if (o != null) {
        ...
        var dialog = $j(o).parent();
        res.x = dialog.css('left').substring(0, dialog.css('left').indexOf("px"));
        res.y = dialog.css('top').substring(0, dialog.css('top').indexOf("px"));
        /*** END CMS ***/
    }
    return res;
}
...
function DIF_begindrag(e, win) {
    /*** CMS ***/
    //window.ensureDragOverlay(window.visiblePopup.parent());
    /*** END CMS ***/
...
function DIF_enddrag(e) {
    /*** CMS ***/
    //window.destroyDragOverlay(window.visiblePopup.parent());
    /*** END CMS ***/
    DIF_dragging = false;
...
function DIF_drag(x, y) {
    var o = DIF_getObjectXY(DIF_iframeObjects[DIF_iframeBeingDragged]);
    /*** CMS ***/
    var dialog = $j(DIF_iframeObjects[DIF_iframeBeingDragged]).parent();
    ...
    if (!DIF_allowDragOffScreen) {
        var win = $j(window);
        var ux = win.width() - dialog.outerWidth() + sl;
        var uy = win.height() - dialog.outerHeight() + st;
        var minX = sl;
        var minY = st;
        if (newPositionX < minX) {
            newPositionX = minX;
        }
        else if (newPositionX > ux) {
            newPositionX = ux;
        }
    ...
    if (!dialog.data('fullScreen')) {

        dialog.css('left', newPositionX + "px");
        dialog.css('top', newPositionY + "px");
        var offset = { left: newPositionX - sl, top: newPositionY - st };
        dialog.data('originalOffset', offset);
        dialog.data('actualOffset', offset);
