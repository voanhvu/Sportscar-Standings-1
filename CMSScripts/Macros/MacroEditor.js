...
function AutoCompleteExtender(codeElem, hintElem, contextElem, quickContextElem, mixedMode, editorTopOffset, editorLeftOffset, ascxMode) {
    ...
    */
    ...
    this.positionDivs = function () {
        ...
        var elemPos = this.getElementPosition(scroller);
        var maxHintsX = elemPos.x + scroller.offsetWidth - this.hintsDiv.offsetWidth - 20;
        ...
        var quickContextLeft = (x <= elemPos.x + scroller.offsetWidth / 2) || (scroller.offsetWidth < 300);
        
        var hintDown = y <= elemPos.y + scroller.offsetHeight / 2;
        ...
        var contextX = x;
        if (!quickContextLeft) {
            contextX -= this.contextDiv.offsetWidth - this.hintsDiv.offsetWidth;
        }

        if (hintDown) {
            y += 25;

            this.contextDiv.style.left = contextX + 'px';
