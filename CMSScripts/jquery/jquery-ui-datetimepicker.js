...
(function($, undefined) {
    ...
    $.extend(CMSDatepicker.prototype, {
        ...
        _updateDatepicker: function(inst) {
            ...
            inst.dpDiv.find('button, .datetime-ui-datepicker-prev, .datetime-ui-datepicker-next, .datetime-ui-datepicker-calendar td a')
            ...
            if (inst == $.cmsdatepicker._curInst && $.cmsdatepicker._datepickerShowing && inst.input &&
	   ...
	   @param  format    string - the desired format of the date
        ...
        _generateHTML: function(inst) {
			...
			'<a class="datetime-ui-datepicker-prev datetime-ui-corner-all" onclick="DP_jQuery_' + dpuuid +
			'.cmsdatepicker._adjustDate(\'#' + inst.id + '\', -' + stepMonths + ', \'M\');"' +
			' title="' + prevText + '"><span class="datetime-ui-icon datetime-ui-icon-circle-triangle-' + (isRTL ? 'e' : 'w') + '"></span></a>' :
			(hideIfNoPrevNext ? '' : '<a class="datetime-ui-datepicker-prev datetime-ui-corner-all datetime-ui-state-disabled" title="' + prevText + '"><span class="datetime-ui-icon datetime-ui-icon-circle-triangle-' + (isRTL ? 'e' : 'w') + '"></span></a>'));
            var nextText = this._get(inst, 'nextText');
            nextText = (!navigationAsDateFormat ? nextText : this.formatDate(nextText,
			this._daylightSavingAdjust(new Date(drawYear, drawMonth + stepMonths, 1)),
			this._getFormatConfig(inst)));
            var next = (this._canAdjustMonth(inst, +1, drawYear, drawMonth) ?
			'<a class="datetime-ui-datepicker-next datetime-ui-corner-all" onclick="DP_jQuery_' + dpuuid +
			'.cmsdatepicker._adjustDate(\'#' + inst.id + '\', +' + stepMonths + ', \'M\');"' +
			' title="' + nextText + '"><span class="datetime-ui-icon datetime-ui-icon-circle-triangle-' + (isRTL ? 'w' : 'e') + '"></span></a>' :
			(hideIfNoPrevNext ? '' : '<a class="datetime-ui-datepicker-next datetime-ui-corner-all datetime-ui-state-disabled" title="' + nextText + '"><span class="datetime-ui-icon datetime-ui-icon-circle-triangle-' + (isRTL ? 'w' : 'e') + '"></span></a>'));
            ...
            // Buttons panel
            var buttonOKStyle = isRTL ? 'style="float:left"' : 'style="float:right"';
            var buttonRestStyle = isRTL ? 'style="float:right"' : 'style="float:left"';
            var okButton = (!inst.inline && displayOK ? '<button type="button" ' + buttonOKStyle + ' class="datetime-ui-datepicker-close datetime-ui-state-default datetime-ui-priority-primary datetime-ui-corner-all" onclick="DP_jQuery_' + dpuuid +
			'.cmsdatepicker._saveAndClose(\'#' + inst.id + '\');' + okCommand + '";">' + this._get(inst, 'closeText') + '</button>' : '');
            var nowButton = this._isInRange(inst, gotoDate) && displayNow ? "<button type=\"button\" " + buttonRestStyle + "class=\"datetime-ui-datepicker-current datetime-ui-state-default datetime-ui-priority-secondary datetime-ui-corner-all\" onClick =\"SelectModalDate_" + inst.id + "('now', '" + inst.id + "'); return false;\"  >" + currentText + '</button>' : '';
            var NAButton = displayNA ? '<button type="button" ' + buttonRestStyle + ' class="datetime-ui-datepicker-current datetime-ui-state-default datetime-ui-priority-secondary datetime-ui-corner-all" onclick="DP_jQuery_' + dpuuid +
            	'.cmsdatepicker._clearDate(\'#' + inst.id + '\');' + okCommand + '">' + this._get(inst, 'NAText') + '</button>' : '';

            var buttonPanel = (showButtonPanel) ? '<div class="datetime-ui-datepicker-buttonpane datetime-ui-widget-content ">' + okButton + nowButton + NAButton + '</div>' : '';
            ...
            for (var row = 0; row < numMonths[0]; row++) {
                ...
                for (var col = 0; col < numMonths[1]; col++) {
                        ...
                        if (numMonths[1] > 1)
                            switch (col) {
                                case 0: calender += ' datetime-ui-datepicker-group-first';
                                    cornerClass = ' datetime-ui-corner-' + (isRTL ? 'right' : 'left'); break;
                                case numMonths[1] - 1: calender += ' datetime-ui-datepicker-group-last';
                                    cornerClass = ' datetime-ui-corner-' + (isRTL ? 'left' : 'right'); break;
                                default: calender += ' datetime-ui-datepicker-group-middle'; cornerClass = ''; break;
                        }
                        calender += '">';
                    }
                    calender += '<div class="datetime-ui-datepicker-header datetime-ui-widget-header datetime-ui-helper-clearfix' + cornerClass + '">' +
					(/all|left/.test(cornerClass) && row == 0 ? (isRTL ? next : prev) : '') +
					(/all|right/.test(cornerClass) && row == 0 ? (isRTL ? prev : next) : '') +
					this._generateMonthYearHeader(inst, drawMonth, drawYear, minDate, maxDate,
					row > 0 || col > 0, monthNames, monthNamesShort) + // draw month headers
					'</div><table class="datetime-ui-datepicker-calendar"><thead>' +
					'<tr>';
                    if (showDaySelector) {
                    ...
                    if (showTimePanel) {
                        ...
                        var seconds = "<td>:</td><td><input class='datetime-ui-time-textbox' type='text' onKeyDown='return DP_jQuery_" + dpuuid + ".cmsdatepicker._checkHours(event,id,59,0);' id='" + inst.id + "_second' value = '" + this._addZero(dt.getSeconds(), zeroSecond) + "'></td>" +
                        ...
                        calender += "<div class=\"RightAlign\">" +
                                   "<table><tr>" + (isRTL ? "" : "<td style=\"width:100%\"/>") + "<td><input class='datetime-ui-time-textbox' type='text' onKeyDown='return DP_jQuery_" + dpuuid + ".cmsdatepicker._checkHours(event,id," + hourMax + "," + hourMin + ");' id='" + inst.id + "_hour' value = '" + this._addZero(hours, zeroHour) + "'></td>" +
                                   "<td><div class='datetime-ui-time-arrow_move datetime-ui-icon datetime-ui-timeicon-n' onclick=\"DP_jQuery_" + dpuuid + ".cmsdatepicker._moveTime ('" + inst.id + "_hour'," + hourMax + "," + hourMin + ",1," + zeroHour + "," + hourShiftID + ");\" ></div><div class='datetime-ui-time-arrow_move datetime-ui-icon datetime-ui-timeicon-s' onclick=\"DP_jQuery_" + dpuuid + ".cmsdatepicker._moveTime ('" + inst.id + "_hour'," + hourMax + "," + hourMin + ",-1," + zeroHour + "," + hourShiftID + ");\"></div></td><td>:</td>" +
        ...
        _generateMonthYearHeader: function(inst, drawMonth, drawYear, minDate, maxDate,
            ...
            var html = '<div class="datetime-ui-datepicker-title">';
            var monthHtml = '';
            // month selection
            if (secondary || !changeMonth)
                ...
                monthHtml += '<select class="datetime-ui-datepicker-month" ' +
				'onchange="DP_jQuery_' + dpuuid + '.cmsdatepicker._selectMonthYear(\'#' + inst.id + '\', this, \'M\');" ' +
				'onclick="DP_jQuery_' + dpuuid + '.cmsdatepicker._clickMonthYear(\'#' + inst.id + '\');"' +
			 	'>';
            ...
            if (secondary || !changeYear)
                ...
                inst.yearshtml += '<select class="datetime-ui-datepicker-year" ' +
                ...
                html += '<select class="datetime-ui-datepicker-year"><option value="' + drawYear + '" selected="selected">' + drawYear + '</option></select>';
                }
            }
