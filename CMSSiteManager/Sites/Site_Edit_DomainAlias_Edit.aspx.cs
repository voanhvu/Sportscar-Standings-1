...
public partial class CMSSiteManager_Sites_Site_Edit_DomainAlias_Edit : SiteManagerPage, ICallbackEventHandler
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!RequestHelper.IsPostBack())
        {
            ...
            {
                ShowError(String.Format(GetString("SiteDomain.RunError"), HTMLHelper.HTMLEncode(collisionSite[0]), HTMLHelper.HTMLEncode(collisionSite[1]), HTMLHelper.HTMLEncode(si.DisplayName)));
            }
        }

        InitializeComponents();

        if ((QueryHelper.GetString("saved", String.Empty) != String.Empty) && !URLHelper.IsPostback())
        {
            ShowChangesSaved();
        }
        ...
        CurrentMaster.Title.HelpTopicName = "edit_and_new_alias";
    }
        ...
        if ((si != null) && (si.Status == SiteStatusEnum.Running))
        {
            ...
            ScriptHelper.RegisterClientScriptBlock(this, typeof(string), "ReceiveServerData",
                                                   ScriptHelper.GetScript("function ReceiveServerData(value) { " +
                                                                          "if ((value == 'ok') || confirm('" + GetString("sitedomain.proceedwithcollision") + "')) { " + ClientScript.GetPostBackEventReference(btnOk, null) + "; } " +
                                                                          "else { return false; } }"));

            btnOk.OnClientClick = Page.ClientScript.GetCallbackEventReference(this, "GetDomainName()", "ReceiveServerData", null) + ";return false;";
        }


        // Initialize culture selector
        cultureSelector.AddDefaultRecord = false;
        cultureSelector.SpecialFields = new string[,] { { GetString("Site_Edit.Automatic"), "" } };
    ...
    private void LoadData()
    {
        if (alias != null)
        {
            ...
            cultureSelector.Value = alias.SiteDefaultVisitorCulture;
        }
    }
