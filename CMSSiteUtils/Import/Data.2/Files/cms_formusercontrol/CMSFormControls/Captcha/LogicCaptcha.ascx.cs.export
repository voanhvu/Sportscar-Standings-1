...
public partial class CMSFormControls_Captcha_LogicCaptcha : FormEngineUserControl, ICaptchaControl
{
    #region "Constants"
    ...
    {
        bool isValid = false;
        string captcha = ValidationHelper.GetString(SessionHelper.GetValue("CaptchaImageText" + ClientID), string.Empty);
    ...
    private void CreateQuestion()
    {
        ...
        // Store in session
        SessionHelper.SetValue("CaptchaImageText" + ClientID, result);
    ...
    private int GetImageQuestion(out string question, Random random)
    {
        ...
        if (line.Length == 2)
        {
            ...
            SessionHelper.SetValue("CaptchaImageText" + ClientID + "Code", code);
    ...
    private string[] SelectLine(Random random, string questions)
    {
        ...
        // Get current culture
        string culture = ValidationHelper.GetString(ViewState["CultureCode"], CMSContext.PreferredCultureCode);
        string sitename = CMSContext.CurrentSiteName;

        // Get or generate cache item name
        string useCacheItemName = CacheHelper.BaseCacheKey + "|" + URLHelper.Url.ToString() + "|" + ClientID + "|" + culture + "|" + UseTextOrImageCaptcha;
        ...
        else
        {
            ...
            if (!CacheHelper.TryGetItem(useCacheItemName, out value))
            {
                ...
                {
                    CacheHelper.Add(useCacheItemName, lines, null, DateTime.Now.AddMinutes(CacheHelper.CacheMinutes(sitename)), System.Web.Caching.Cache.NoSlidingExpiration);
                }
            }
            else
            {
