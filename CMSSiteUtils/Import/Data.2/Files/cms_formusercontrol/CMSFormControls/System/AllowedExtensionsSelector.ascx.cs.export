...
public partial class CMSFormControls_System_AllowedExtensionsSelector : FormEngineUserControl, ICallbackEventHandler
{
    ...
    public override bool IsValid()
    {
        ...
        {
            this.ValidationError += String.Format(GetString("formcontrol.missingcolumn"), "extensions", GetString("general.text"));
            isValid = false;
        }

        if (!ContainsColumn("allowed_extensions"))
        {
            this.ValidationError += String.Format(GetString("formcontrol.missingcolumn"), "allowed_extensions", GetString("general.text"));
            isValid = false;
        }
