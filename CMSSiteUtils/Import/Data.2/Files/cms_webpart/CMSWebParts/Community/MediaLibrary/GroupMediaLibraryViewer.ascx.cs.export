...
public partial class CMSWebParts_Community_MediaLibrary_GroupMediaLibraryViewer : CMSAbstractWebPart
{
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            {
                GroupInfo gi = GroupInfoProvider.GetGroupInfo(GroupName, (String.IsNullOrEmpty(SiteName) ? CMSContext.CurrentSiteName : SiteName));
                if (gi != null)
                {
            ...
            {
                repLibraries.ItemTemplate = CMSDataProperties.LoadTransformation(this, TransformationName, false);
            }
            if (!String.IsNullOrEmpty(AlternatingItemTransformationName))
            {
                repLibraries.AlternatingItemTemplate = CMSDataProperties.LoadTransformation(this, AlternatingItemTransformationName, false);
            }
            if (!String.IsNullOrEmpty(FooterTransformationName))
            {
                repLibraries.FooterTemplate = CMSDataProperties.LoadTransformation(this, FooterTransformationName, false);
            }
            if (!String.IsNullOrEmpty(HeaderTransformationName))
            {
                repLibraries.HeaderTemplate = CMSDataProperties.LoadTransformation(this, HeaderTransformationName, false);
            }
            if (!String.IsNullOrEmpty(SeparatorTransformationName))
            {
                repLibraries.SeparatorTemplate = CMSDataProperties.LoadTransformation(this, SeparatorTransformationName, false);
            }
            ...
            if (!DataHelper.DataSourceIsEmpty(repLibraries.DataSource))
            {
