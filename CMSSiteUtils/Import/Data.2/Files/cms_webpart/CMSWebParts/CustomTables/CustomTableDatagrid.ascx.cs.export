...
public partial class CMSWebParts_CustomTables_CustomTableDataGrid : CMSAbstractWebPart
{
    ...
    private DataClassInfo DataClassInfo
    {
        get
        {
            ...
            {
                mDataClassInfo = DataClassInfoProvider.GetDataClass(CustomTable);
            }
        ...
        else
        {
            ...
            if (CheckPermissions)
            {
                ...
                if (!CMSContext.CurrentUser.IsAuthorizedPerResource("CMS.CustomTables", "Read") &&
                    !CMSContext.CurrentUser.IsAuthorizedPerClassName(DataClassInfo.ClassName, "Read"))
                {
                    return;
                }
            }
        ...
        if ((e.Item.ItemType == ListItemType.Item) || (e.Item.ItemType == ListItemType.AlternatingItem))
        {
            ...
            if (dataItem.GetType() == typeof(DataRowView))
            {
                ...
                for (int i = 0; i < columnCount; i++)
                {
                    ...
                    if (((column.GetType() == typeof(BoundColumn)) && !gridItems.AutoGenerateColumns) || ((column.GetType() == typeof(DateTime)) && gridItems.AutoGenerateColumns))
                    {
                    ...
                    else if (gridItems.AutoGenerateColumns && (column.GetType() == typeof(Guid)))
                    {
                        --j;
                    }
