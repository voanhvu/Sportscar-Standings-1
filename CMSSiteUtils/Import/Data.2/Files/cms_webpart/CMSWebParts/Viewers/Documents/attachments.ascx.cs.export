...
public partial class CMSWebParts_Viewers_Documents_attachments : CMSAbstractWebPart
{
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            if ((EnableAddButton) && (CMSContext.ViewMode == ViewModeEnum.Edit))
            {
                DataClassInfo dci = DataClassInfoProvider.GetDataClass(ClassNames);
                if (dci != null)
                {
                    ltrAddButton.Text = "<a href=\"javascript:NewDocument( " + CMSContext.CurrentDocument.NodeID + ", " + dci.ClassID + ");\" class=\"CMSEditModeButtonAdd\">" + GetString("AttachmentsWebPart.Add") + "</a><br />";
                }
            }
        }
    }
