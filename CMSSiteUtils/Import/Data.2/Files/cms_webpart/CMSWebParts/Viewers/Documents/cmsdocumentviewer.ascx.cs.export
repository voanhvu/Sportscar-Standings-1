...
public partial class CMSWebParts_Viewers_Documents_cmsdocumentviewer : CMSAbstractWebPart
{
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            string mTransformationName = DataHelper.GetNotEmpty(GetValue("TransformationName"), string.Empty);

            if (!string.IsNullOrEmpty(mTransformationName))
            {
                // Get transformation info
                TransformationInfo mTransformation = TransformationInfoProvider.GetTransformation(mTransformationName);
                if ((mTransformation == null) && (PageManager.ViewMode != ViewModeEnum.Design))
                {
                    throw new Exception("[WebPart.CMSDocumentViever]: Transformation " + mTransformationName + " not found!");
                }

                // Get type of transformation
                if (mTransformation != null)
                {
                    if (mTransformation.TransformationType == TransformationTypeEnum.Xslt)
                    {
