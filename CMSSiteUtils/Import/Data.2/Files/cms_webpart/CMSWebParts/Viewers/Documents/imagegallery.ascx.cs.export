...
public partial class CMSWebParts_Viewers_Documents_ImageGallery : CMSAbstractWebPart
{
    ...
    public string Path
    {
        get
        {
            return DataHelper.GetNotEmpty(ValidationHelper.GetString(GetValue("Path"), lstImages.Path), lstImages.Path);
        }
        set
        {
            SetValue("Path", value);
    ...
    /// </summary>
    void pagerElem_OnDataSourceChanged(object sender, EventArgs e)
    {
        if (QueryHelper.Contains("imagepath"))
        {
            ...
            int currentPage = PagerControl.GetItemPage("NodeAliasPath = '" + SqlHelperClass.GetSafeQueryString(path, false) + "'");
            ...
            if (nextItem != null)
            {
                lnkNext.NavigateUrl = URLHelper.AddParameterToUrl(URLRewriter.CurrentURL, "imagepath", (string)nextItem["NodeAliasPath"]);
            }
            ...
            if (prevItem != null)
            {
                lnkPrevious.NavigateUrl = URLHelper.AddParameterToUrl(URLRewriter.CurrentURL, "imagepath", (string)prevItem["NodeAliasPath"]);
            }

            lnkThumbnails.Text = GetString("ImageGallery.Thumbnails");
            lnkThumbnails.NavigateUrl = URLHelper.RemoveParameterFromUrl(URLRewriter.CurrentURL, "imagepath");
            ...
            {
                string url = URLHelper.AddParameterToUrl(URLRewriter.CurrentURL, "imagepath", (string)dr["NodeAliasPath"]);
