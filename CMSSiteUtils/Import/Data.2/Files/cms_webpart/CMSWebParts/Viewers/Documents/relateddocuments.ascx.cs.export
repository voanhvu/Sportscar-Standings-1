    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            if (currentPage != null)
            {
                ...
                // Get documents
                DataSet ds = TreeHelper.GetDocuments(CMSContext.CurrentSiteName, "/%", currentPage.DocumentCulture, true, "", WhereCondition, OrderBy, -1, SelectOnlyPublished, currentPage.NodeGUID, RelationshipName, CurrentDocumentIsOnTheLeftSide, SelectTopN, Columns);
                repElem.DataSource = ds;

                repElem.Path = "/%";
                repElem.MaxRelativeLevel = -1;
                repElem.ClassNames = "CMS.Root";
