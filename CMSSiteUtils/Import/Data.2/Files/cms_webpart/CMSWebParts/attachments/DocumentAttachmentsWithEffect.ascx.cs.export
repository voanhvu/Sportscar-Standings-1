...
public partial class CMSWebParts_Attachments_DocumentAttachmentsWithEffect : CMSAbstractWebPart
{
    ...
    public string PagingMode
    {
        get
        {
            return ValidationHelper.GetString(this.GetValue("PagingMode"), "querystring");
        }
        set
        {
            if (value != null)
            {
                this.SetValue("PagingMode", value);
                switch (value.ToLower())
                {
                    case "postback": ucPager.PagerMode = UniPagerMode.PostBack; break;
                    default: ucPager.PagerMode = UniPagerMode.Querystring; break;
                }
            }
        }
    }
    ...
    public override string CacheItemName
    {
        get
        {
            return ValidationHelper.GetString(GetValue("CacheItemName"), ucDataSource.CacheItemName);
        }
        set
        {
    ...
    public override string CacheDependencies
    {
        get
        {
            return ValidationHelper.GetString(GetValue("CacheDependencies"), ucDataSource.CacheDependencies);
        }
        set
        {
    ...
    public override int CacheMinutes
    {
        get
        {
            return ValidationHelper.GetInteger(GetValue("CacheMinutes"), ucDataSource.CacheMinutes);
        }
        set
        {
    ...
    public string Path
    {
        get
        {
            return ValidationHelper.GetString(GetValue("Path"), ucDataSource.Path);
        }
        set
        {
            SetValue("Path", value);
    ...
    public bool IncludeJQuery
    {
        ...
        set
        {
            ...
            ucRepeater.IncludeJQuery = value;
        }
    }
    ...
    private void SetupTemplates()
    {
        ...
        ucRepeater.ItemTemplate = CMSDataProperties.LoadTransformation(this, TransformationName, false);

        if (!string.IsNullOrEmpty(AlternatingItemTransformationName))
        {
            ucRepeater.AlternatingItemTemplate = CMSDataProperties.LoadTransformation(this, AlternatingItemTransformationName, false);
        }
        if (!string.IsNullOrEmpty(FooterTransformationName))
        {
            ucRepeater.FooterTemplate = CMSDataProperties.LoadTransformation(this, FooterTransformationName, false);
        }
        if (!string.IsNullOrEmpty(HeaderTransformationName))
        {
            ucRepeater.HeaderTemplate = CMSDataProperties.LoadTransformation(this, HeaderTransformationName, false);
        }
        if (!string.IsNullOrEmpty(SeparatorTransformationName))
        {
            ucRepeater.SeparatorTemplate = CMSDataProperties.LoadTransformation(this, SeparatorTransformationName, false);
        }
        ...
        {
            ucPager.PageNumbersTemplate = CMSDataProperties.LoadTransformation(ucPager, PagesTemplate, false);
        }

        if (!string.IsNullOrEmpty(CurrentPageTemplate))
        {
            ucPager.CurrentPageTemplate = CMSDataProperties.LoadTransformation(ucPager, CurrentPageTemplate, false);
        }

        if (!string.IsNullOrEmpty(SeparatorTemplate))
        {
            ucPager.PageNumbersSeparatorTemplate = CMSDataProperties.LoadTransformation(ucPager, SeparatorTemplate, false);
        }

        if (!string.IsNullOrEmpty(FirstPageTemplate))
        {
            ucPager.FirstPageTemplate = CMSDataProperties.LoadTransformation(ucPager, FirstPageTemplate, false);
        }

        if (!string.IsNullOrEmpty(LastPageTemplate))
        {
            ucPager.LastPageTemplate = CMSDataProperties.LoadTransformation(ucPager, LastPageTemplate, false);
        }

        if (!string.IsNullOrEmpty(PreviousPageTemplate))
        {
            ucPager.PreviousPageTemplate = CMSDataProperties.LoadTransformation(ucPager, PreviousPageTemplate, false);
        }

        if (!string.IsNullOrEmpty(NextPageTemplate))
        {
            ucPager.NextPageTemplate = CMSDataProperties.LoadTransformation(ucPager, NextPageTemplate, false);
        }

        if (!string.IsNullOrEmpty(PreviousGroupTemplate))
        {
            ucPager.PreviousGroupTemplate = CMSDataProperties.LoadTransformation(ucPager, PreviousGroupTemplate, false);
        }

        if (!string.IsNullOrEmpty(NextGroupTemplate))
        {
            ucPager.NextGroupTemplate = CMSDataProperties.LoadTransformation(ucPager, NextGroupTemplate, false);
        }

        if (!string.IsNullOrEmpty(LayoutTemplate))
        {
            ucPager.LayoutTemplate = CMSDataProperties.LoadTransformation(ucPager, LayoutTemplate, false);
        }
