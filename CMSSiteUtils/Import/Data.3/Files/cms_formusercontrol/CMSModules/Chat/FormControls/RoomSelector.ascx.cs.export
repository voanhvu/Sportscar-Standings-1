...
public partial class CMSModules_Chat_FormControls_RoomSelector : FormEngineUserControl
{
    ...
    {
        usRooms.IconPath = UIHelper.GetImageUrl(Page, "Objects/CMS_ChatRoom/object.png");

        //Display only current site's rooms
        usRooms.WhereCondition = SqlHelperClass.AddWhereCondition(usRooms.WhereCondition, string.Format("((ChatRoomSiteID = {0}) OR ChatRoomSiteID IS NULL) AND (ChatRoomPassword = '' ) AND (ChatRoomIsOneToOne = 0) AND (ChatRoomPrivate = 0)", (SiteID > 0) ? SiteID : CMSContext.CurrentSiteID));
        ...
        if (HideDisabledRooms)
        {
            ...
            usRooms.WhereCondition = SqlHelperClass.AddWhereCondition(usRooms.WhereCondition, where);
        }

        // Add aditional rooms
        if ((AdditionalRooms != null) && (AdditionalRooms.Length > 0))
        {
            usRooms.WhereCondition = SqlHelperClass.AddWhereCondition(usRooms.WhereCondition, SqlHelperClass.GetWhereCondition("ChatRoomID", AdditionalRooms), "OR");
        }

        // Control where condition
        if (!String.IsNullOrEmpty(WhereCondition))
        {
            usRooms.WhereCondition = SqlHelperClass.AddWhereCondition(usRooms.WhereCondition, WhereCondition);
        }
