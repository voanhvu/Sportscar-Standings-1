...
{    
    ...
    public string MyAccountLinkUrl
    {
        get
        {
            return DataHelper.GetNotEmpty(GetValue("MyAccountLinkUrl"), SettingsKeyProvider.GetStringValue(SiteName + ".CMSMyAccountURL"));
        }
        set
        {
    ...
    private void SetShoppingCartPreviewText()
    {
        ...
        // Is cart empty?
        ShoppingCartInfo sc = ECommerceContext.CurrentShoppingCart;
        bool cartEmpty = (sc != null) && (sc.CartItems.Count <= 0);
        
        if (!cartEmpty || (cartEmpty && !ShowEmptyShoppingCartText))
        {
            // Display title
