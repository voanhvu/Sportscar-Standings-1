...
public partial class CMSWebParts_Forums_ForumFavorites : CMSAbstractWebPart
{
    ...
    public string DeleteImageUrl
    {
        get
        {
            return URLHelper.ResolveUrl(DataHelper.GetNotEmpty(GetValue("DeleteImageUrl"), GetImageUrl("CMSModules/CMS_Forums/delete.png")));
        }
        set
        {
    ...
    private void BindData()
    {
        if (CMSContext.CurrentUser != null)
        {
            ...
            using (CachedSection<DataSet> cs = new CachedSection<DataSet>(ref ds, CacheMinutes, true, CacheItemName, "forumfavorites", userId, siteId))
            {
                if (cs.LoadData)
                {
                    // Get the data
