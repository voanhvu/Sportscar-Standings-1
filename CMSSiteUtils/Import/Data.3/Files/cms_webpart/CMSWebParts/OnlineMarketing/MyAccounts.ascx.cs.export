...
public partial class CMSWebParts_OnlineMarketing_MyAccounts : CMSAbstractWebPart
{
    ...
    private void RegisterScripts()
    {
        ...
        script.AppendLine(@"function EditAccount(accountID) {  modalDialog('" + ResolveUrl(ACCOUNT_DETAIL_DIALOG) + @"?accountid=' + accountID, 'AccountDetail', '1024px', '700px'); }");
        // Register script to open dialogs for contact editing
        script.AppendLine(@" function EditContact(contactID) {  modalDialog('" + ResolveUrl(CONTACT_DETAIL_DIALOG) + @"?contactid=' + contactID, 'ContactDetail', '1024px', '700px'); }");
