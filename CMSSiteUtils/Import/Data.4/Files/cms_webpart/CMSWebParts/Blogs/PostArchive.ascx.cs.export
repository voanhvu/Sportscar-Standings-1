...
public partial class CMSWebParts_Blogs_PostArchive : CMSAbstractWebPart
{
    ...
    protected void rptPostArchive_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        using (CachedSection<int> cs = new CachedSection<int>(ref count, CacheMinutes, true, CacheItemName, "postarchivecount", CMSContext.CurrentSiteName, rptPostArchive.Path, CMSContext.PreferredCultureCode, parentId, selectOnlyPublished))
        {
            if (cs.LoadData)
            {
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            // Try to get data from cache
            using (CachedSection<TreeNode> cs = new CachedSection<TreeNode>(ref blogNode, CacheMinutes, true, CacheItemName, "postarchivenode", currentPage.NodeAliasPath, CMSContext.CurrentSiteName, selectOnlyPublished))
            {
                if (cs.LoadData)
                {
