function ChatRoomMessagesWP(opt) {
    ...
    this.Options = jQuery.extend(that.Options, opt);
    ...
    var msgDivPrefix = "ChatMessage_ID_",
        ...
        container = jQuery(that.Options.container),
        oneToOne = false,
        support = false,
    ...
    this.ProcessResponse = function (messages) {
        ...
        for (var i = 0; i < messages.length; i++) {
            ...
            // Non-system message 
                ...
                if (ChatManager.Login.UserID != msg.AuthorID) {
    ...
    function addNewMessage(msg) {
        ...
        if (that.Options.direction == 1) {
            jQuery.tmpl(chatMessageTemplateB, [msg]).appendTo(content);
        }
        else {
            jQuery.tmpl(chatMessageTemplateB, [msg]).prependTo(content);
        }
    ...
    function modifyMessage(msg, adminChangedOnly) {
        ...
        messageDiv.empty();
        jQuery.tmpl(chatMessageTemplateB, [msg]).prependTo(messageDiv);
        parseBBCodeAndSmileys(msg);
    };
    ...
    function rejectMessage(msg) {
        ...
        if (msgDiv.length == 0) {
            ...
            msgDiv.empty();
            jQuery.tmpl(chatMessageTemplateB, [msg]).prependTo(msgDiv);
        }
        parseBBCodeAndSmileys(msg);
    };
    ...
    function adjustScrollbar() {
        container.scrollTop(content.height() - container.height());
    }
    

    // Checks where the scrollbar is and if the message window should be scrolled down
    function checkScrollbar() {
        var paddingTop = (jQuery.browser.msie || jQuery.browser.mozilla) ? container.css("padding-top").replace("px", "") : 0; 
    ...
    function Inicialize() {
        ...
        jQuery.template(chatMessageTemplateB, that.Options.chatMessageTemplate);
