...
public partial class CMSWebParts_TaggingCategories_CategoryList : CMSAbstractWebPart
{
    ...
    public string AliasPath
    {
        get
        {
            ...
            CMS.DocumentEngine.TreeNode currentDoc = CMSContext.CurrentDocument;

            if (currentDoc != null)
            {
                if (path == "")
                {
                    return currentDoc.NodeAliasPath.TrimEnd('/') + "/%";
                }

                return CMSContext.ResolveCurrentPath(path);
            }
    ...
    public void rptCategoryList_ItemDataBound(object sender, RepeaterItemEventArgs e)
    {
        ...
        if (e.Item.Controls.Count > 0)
        {
            ...
            int id = ValidationHelper.GetInteger(drv.Row["CategoryID"], 0);
            string name = drv.Row["CategoryDisplayName"].ToString();
