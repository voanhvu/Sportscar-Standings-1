...
public partial class CMSWebParts_TaggingCategories_TagCloud : CMSAbstractWebPart
{
    ...
    public string TagGroupName
    {
        get
        {
            ...
            if (String.IsNullOrEmpty(tagGroupName))
            {
                ...
                if (CMSContext.CurrentDocument.DocumentTagGroupID != 0)
                {
                    tagGroupId = CMSContext.CurrentDocument.DocumentTagGroupID;
                }
                else
                {
                    tagGroupId = ValidationHelper.GetInteger(CMSContext.CurrentDocument.GetInheritedValue("DocumentTagGroupID", SiteInfoProvider.CombineWithDefaultCulture(CMSContext.CurrentSiteName)), 0);
                }

                // Get tag group information
                TagGroupInfo tgi = TagGroupInfoProvider.GetTagGroupInfo(tagGroupId);

                if (tgi != null)
                {
                    // Get tag group name
                    tagGroupName = tgi.TagGroupName;
                }
            }
    ...
    public bool CombineWithDefaultCulture
    {
        get
        {
            return ValidationHelper.GetBoolean(GetValue("CombineWithDefaultCulture"), SettingsKeyProvider.GetBoolValue(SiteName + ".CMSCombineWithDefaultCulture"));
        }
        set
        {
    ...
    {
        string useCacheItemName = DataHelper.GetNotEmpty(CacheItemName, CacheHelper.BaseCacheKey + "|" + URLHelper.Url.ToString() + "|" + ClientID);
    ...
    protected void SetupControl()
    {
        ...
        else
        {
            try
            {
                ...
                using (CachedSection<string> cs = new CachedSection<string>(ref renderedTags, CacheMinutes, true, CacheItemName, "tagcloud", TagGroupName, OrderBy, SelectTopN, DocumentListPath, TagSeparator, QueryStringName, MaxTagSize, MinTagSize, "documents", siteName, aliasPath, CacheHelper.GetCultureCacheKey(cultureCode), CombineWithDefaultCulture, WhereCondition, SelectOnlyPublished, MaxRelativeLevel))
                {
                    if (cs.LoadData)
                    {
