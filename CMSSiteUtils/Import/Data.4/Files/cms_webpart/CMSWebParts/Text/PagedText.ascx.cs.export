...
public partial class CMSWebParts_Text_pagedtext : CMSAbstractWebPart
{
    ...
    protected void LoadData()
    {
        ...
        else
        {
            ...
            using (CachedSection<TreeNode> cs = new CachedSection<TreeNode>(ref node, CacheMinutes, true, CacheItemName, "pagedtext", CacheHelper.GetBaseCacheKey(CheckPermissions, false), SiteName, Path, CultureCode, CombineWithDefaultCulture, SelectOnlyPublished))
            {
                if (cs.LoadData)
                {
