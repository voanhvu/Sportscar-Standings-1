 ...
 debug+="5";
    ...
    private void get_detail()
    {
        ...
        if (dtb != null)
        {
            ...
            if (n_row > 0 && nparam < 3)
            {
                ...
                if (Request.Params["searchtext"] == null)
                {
                    ...
                    Session["path_result"] = "/Search/" + path;
            ...
            string custompath = "/Search/" + class_;
            ...
            link_detail = "/Search" + (class_ != null ? "/" + class_ : "") + "/" + dr["Year"].ToString() + "-" + CarzHelpers.URLEncode(dr["Make"].ToString()) + "-" + CarzHelpers.URLEncode(dr["Model"].ToString(), true) ;
            ...
            string path1 = "/Search/" + class_ + "/" + dr["Year"].ToString() + "-" + CarzHelpers.URLEncode(dr["Make"].ToString()) + "-" + CarzHelpers.URLEncode(dr["Model"].ToString(), true);
            ...
            string path2 = "/Search/" + class_ + "/" + y2 + "-" + CarzHelpers.URLEncode(m2) + "-" + CarzHelpers.URLEncode(md2, true);
            string path3 = "/Search/" + class_ + "/" + y3 + "-" + CarzHelpers.URLEncode(m3) + "-" + CarzHelpers.URLEncode(md3, true);
     ...
     "<p>Best Top Speed: " + Parse_double(dr["BestTopSpeed"].ToString()).ToString("0.0") + " mph </p>"
                ...
                LiteralResultDetail.Text = LiteralResultDetail.Text.Replace("0482", (preview == "" && next == "" ? "" : back) + (preview != "" ? "<  <a title='Prev' href='/Search/" + class_ + "/" + preview + "'>Prev</a>&nbsp;&nbsp;&nbsp;" : "") + (next != "" ? " <a title='Next' href='/Search/" + class_ + "/" + next + "'>Next > </a> " : ""));
    ...
    private void getimage_class(string cls)
    {
        ...
        if (!DataHelper.DataSourceIsEmpty(ds))
        {
            ...
            foreach (DataRow ItemRow in ds.Tables[0].Rows)
            {
                ...
                if (ItemRow["MenuItemName"].ToString() == class_.Replace("-Cars", "").Replace("-", " "))
                {
                    ...
                    if (searchtext != "")
                        LiteralTitle.Text = string.Format("<h1>{0}</h1>", "Cars matching " + (classname != "" ? CarzHelpers.URLDecode(classname.Replace("-", " ")) + " " + title : CarzHelpers.URLDecode(cls.Replace("-", " "))));
                    else
                        LiteralTitle.Text = string.Format("<h1>{0}</h1>", "Standings for " + (classname != "" ? CarzHelpers.URLDecode(classname.Replace("-", " ")) + " " + title : CarzHelpers.URLDecode(cls.Replace("-", " "))));
        ...
        if (searchtext != "") LiteralTitle.Text = string.Format("<h1>{0}</h1>", "Cars matching " + searchtext + " " + title);
    ...
    protected void GridViewResult_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            path_STT = "<a href='/Search/" + classname + CarzHelpers.URLEncode(y) + "-" + CarzHelpers.URLEncode(m) + "-" + CarzHelpers.URLEncode(md, true) + "' title='" + rank + "'>" + rank + "</a>";
            e.Row.Cells[0].Text = path_STT;

            path_STT = "<a href='/Search/" + CarzHelpers.URLEncode(y) + "' title='" + e.Row.Cells[1].Text + "'>" + e.Row.Cells[1].Text + "</a>";
            e.Row.Cells[1].Text = path_STT;

            path_STT = "<a href='/Search/" + CarzHelpers.URLEncode(m) + "' title='" + e.Row.Cells[2].Text + "'>" + e.Row.Cells[2].Text + "</a>";
            e.Row.Cells[2].Text = path_STT;

            path_STT = "<a href='/Search/" + classname + CarzHelpers.URLEncode(y) + "-" + CarzHelpers.URLEncode(m) + "-" + CarzHelpers.URLEncode(md, true) + "' title='" + e.Row.Cells[3].Text + "'>" + e.Row.Cells[3].Text + "</a>";
    ...
    protected override void OnInit(EventArgs e)
    {
        ...
        if (Request.Params["searchtext"] == null)
        {
            ...
            CMSContext.CurrentTitle = "Search - " + (name.Replace(".aspx", "") == "" ? "All" : name.Replace(".aspx", "").Replace(" All", ""));
        }
        else
        {
            CMSContext.CurrentTitle = "Custom Search - " + CarzHelpers.URLDecode(Request.Params["searchtext"].ToString().Replace(".aspx", ""));
