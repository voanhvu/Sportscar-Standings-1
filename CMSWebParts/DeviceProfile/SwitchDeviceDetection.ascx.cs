...
public partial class CMSWebParts_DeviceProfile_SwitchDeviceDetection : CMSAbstractWebPart
{
    ...
    {                 
        ...
        if ((CookieHelper.CurrentCookieLevel < CookieLevel.Essential) || (String.IsNullOrEmpty(CMSContext.CurrentDeviceProfileName) && String.IsNullOrEmpty(DeviceProfileInfoProvider.GetOriginalCurrentDevicProfileName(CurrentSite.SiteName))))
        {
            pnlContent.Visible = false;
            return;
        } 

        bool useDesktop = (ViewMode == ViewModeEnum.LiveSite) ? ShowDesktopVersion : String.IsNullOrEmpty(CMSContext.CurrentDeviceProfileName);
