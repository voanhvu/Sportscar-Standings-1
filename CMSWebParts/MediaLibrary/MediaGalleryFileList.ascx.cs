...
public partial class CMSWebParts_MediaLibrary_MediaGalleryFileList : CMSAbstractWebPart
{
    ...
    protected void SetupControlLoad()
    {
        ...
        if (mli != null)
        {
            mMediaLibraryRoot = MediaLibraryHelper.GetMediaRootFolderPath(CMSContext.CurrentSiteName) + mli.LibraryFolder;
            mMediaLibraryUrl = URLHelper.GetAbsoluteUrl("~/" + CMSContext.CurrentSiteName + "/media/" + mli.LibraryFolder);
        }
        ...
        else
        {
            ...
            if (mli != null)
            {
                ...
                if (fid > 0)
                {
                    ...
                    {
                        repItems.ItemTemplate = CMSDataProperties.LoadTransformation(this, SelectedItemTransformationName, false);
                    }
                }
                else
                {
                    ...
                    {
                        repItems.ItemTemplate = CMSDataProperties.LoadTransformation(this, TransformationName, false);
                    }
                }
                if (!String.IsNullOrEmpty(FooterTransformationName))
                {
                    repItems.FooterTemplate = CMSDataProperties.LoadTransformation(this, FooterTransformationName, false);
                }
                if (!String.IsNullOrEmpty(HeaderTransformationName))
                {
                    repItems.HeaderTemplate = CMSDataProperties.LoadTransformation(this, HeaderTransformationName, false);
                }
                if (!String.IsNullOrEmpty(SeparatorTransformationName))
                {
                    repItems.SeparatorTemplate = CMSDataProperties.LoadTransformation(this, SeparatorTransformationName, false);
                }
