    ...
    protected void SetupControl()
    {
        ...
        else
        {
            ...
            if ((transLoaded) || (!String.IsNullOrEmpty(Path)))
            {
                ...
                using (CachedSection<DataSet> cs = new CachedSection<DataSet>(ref users, CacheMinutes, true, CacheItemName, "onlineusers", CMSContext.CurrentSiteName, SelectTopN, Columns, Path))
                {
                    if (cs.LoadData)
                    {
                        // Get the data
            ...
            // Try to get data from cache
            using (CachedSection<string> cs = new CachedSection<string>(ref numbers, CacheMinutes, true, cacheItemNameNumbers, "onlineusersnumber", CMSContext.CurrentSiteName, Path))
            {
                if (cs.LoadData)
                {
                    // Get the data
